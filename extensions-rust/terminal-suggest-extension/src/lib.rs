...
/// Terminal command suggestions extension.
/// Provides command suggestions in the terminal based on shell integration.
pub struct TerminalSuggestExtension;

impl SidexExtension for TerminalSuggestExtension {
    ...
    fn get_activation_events() -> Vec<String> {
        vec!["onTerminalShellIntegrationReady".to_string()]
    }
    ...
    fn execute_command(_: String, _: String) -> Result<String, String> {
        Err("no commands available".into())
    }
...
sidex_extension_sdk::export_extension!(TerminalSuggestExtension);
