...
pub struct GruntExtension;

impl SidexExtension for GruntExtension {
    ...
    fn get_activation_events() -> Vec<String> {
        vec!["workspaceContains:Gruntfile.js".to_string()]
    }
    ...
    fn get_task_types() -> Vec<TaskDefinition> {
        vec![TaskDefinition {
            type_: "grunt".to_string(),
            label: "Grunt Task".to_string(),
            description: "Run a Grunt task".to_string(),
            ..Default::default()
        }]
    }
...
sidex_extension_sdk::export_extension!(GruntExtension);
