...
pub struct GulpExtension;

impl SidexExtension for GulpExtension {
    ...
    fn get_activation_events() -> Vec<String> {
        vec!["workspaceContains:gulpfile.js".to_string()]
    }
    ...
    fn get_semantic_tokens_legend() -> Option<SemanticTokensLegend> { None }
    fn provide_completion(_: DocumentContext, _: Position) -> Option<CompletionList> { None }
    fn provide_completion_item_resolve(_: String, _: Option<u32>, _: Option<String>) -> Option<CompletionList> { None }
    fn provide_hover(_: DocumentContext, _: Position) -> Option<HoverResult> { None }
    fn provide_definition(_: DocumentContext, _: Position) -> Vec<Location> { vec![] }
    fn provide_references(_: DocumentContext, _: Position) -> Vec<Location> { vec![] }
    fn provide_document_symbols(_: DocumentContext) -> Vec<DocumentSymbol> { vec![] }
    fn provide_formatting(_: DocumentContext, _: u32, _: bool) -> Vec<TextEdit> { vec![] }
    fn provide_range_formatting(_: DocumentContext, _: Range, _: u32, _: bool) -> Vec<TextEdit> { vec![] }
    fn provide_on_type_formatting(_: DocumentContext, _: Position, _: String, _: u32, _: bool) -> Vec<TextEdit> { vec![] }
    fn provide_signature_help(_: DocumentContext, _: Position) -> Option<SignatureHelpResult> { None }
    fn provide_document_highlights(_: DocumentContext, _: Position) -> Vec<DocumentHighlight> { vec![] }
    fn provide_rename(_: DocumentContext, _: Position, _: String) -> Option<RenameResult> { None }
    fn prepare_rename(_: DocumentContext, _: Position) -> Option<RenameLocation> { None }
    fn provide_inlay_hints(_: DocumentContext, _: Range) -> Vec<InlayHint> { vec![] }
    fn provide_inlay_hint_resolve(_: Position, _: String, _: Option<u32>) -> Option<InlayHint> { None }
    fn provide_document_links(_: DocumentContext) -> Vec<DocumentLink> { vec![] }
    fn provide_document_link_resolve(_: Range, _: Option<String>) -> Option<DocumentLink> { None }
    fn provide_selection_ranges(_: DocumentContext, _: Vec<Position>) -> Vec<SelectionRange> { vec![] }
    fn provide_semantic_tokens(_: DocumentContext) -> Option<SemanticTokens> { None }
    ...
    fn provide_document_colors(_: DocumentContext) -> Vec<ColorInfo> { vec![] }
    fn provide_color_presentation(_: DocumentContext, _: ColorInfo, _: Range) -> Vec<TextEdit> { vec![] }
    fn provide_folding_ranges(_: DocumentContext) -> Vec<FoldingRange> { vec![] }
    fn provide_code_actions(_: DocumentContext, _: Range, _: Vec<Diagnostic>) -> Vec<CodeAction> { vec![] }
    fn provide_code_action_resolve(_: String, _: Option<String>, _: Option<String>) -> Option<CodeAction> { None }
    fn provide_code_lenses(_: DocumentContext) -> Vec<CodeLens> { vec![] }
    fn provide_code_lens_resolve(_: Range, _: Option<String>, _: Option<String>) -> Option<CodeLens> { None }
    fn provide_type_definition(_: DocumentContext, _: Position) -> Vec<Location> { vec![] }
    fn provide_implementation(_: DocumentContext, _: Position) -> Vec<Location> { vec![] }
    fn provide_declaration(_: DocumentContext, _: Position) -> Vec<Location> { vec![] }
    fn provide_workspace_symbols(_: String) -> Vec<DocumentSymbol> { vec![] }
    ...
    fn execute_command(_: String, _: String) -> Result<String, String> { Err("not supported".into()) }
    fn get_tree_children(_: String, _: Option<String>) -> Vec<TreeItem> { vec![] }
...
sidex_extension_sdk::export_extension!(GulpExtension);
