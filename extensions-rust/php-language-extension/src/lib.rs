...
pub struct PhpLanguageExtension;

impl SidexExtension for PhpLanguageExtension {
    ...
    fn get_name() -> String {
        "PHP Language Features".to_string()
    }
    ...
    fn get_activation_events() -> Vec<String> {
        vec![
            "onLanguage:php".to_string(),
            "onLanguage:php4".to_string(),
            "onLanguage:php5".to_string(),
            "onLanguage:php7".to_string(),
            "onLanguage:php8".to_string(),
        ]
    }
    ...
    fn provide_completion(ctx: DocumentContext, pos: Position) -> Option<CompletionList> {
        if !is_php_language(&ctx.language_id) {
            return None;
        }
...
fn is_php_language(language_id: &str) -> bool {
    matches!(language_id, "php" | "php4" | "php5" | "php7" | "php8")
}
...
sidex_extension_sdk::export_extension!(PhpLanguageExtension);
