...
/// Provides: completion, hover, formatting, document symbols, folding ranges,
/// document links, and color presentation for HTML/HTM files.
pub struct HtmlLanguageExtension;

impl SidexExtension for HtmlLanguageExtension {
    ...
    fn get_name() -> String {
        "HTML Language Features".to_string()
    }
    ...
    fn get_activation_events() -> Vec<String> {
        vec![
            "onLanguage:html".to_string(),
            "onLanguage:htm".to_string(),
        ]
    }
    ...
    fn provide_completion(ctx: DocumentContext, _pos: Position) -> Option<CompletionList> {
        if !is_html(&ctx.language_id) {
            return None;
        }
        None
    }

    fn provide_hover(ctx: DocumentContext, _pos: Position) -> Option<HoverResult> {
        if !is_html(&ctx.language_id) {
            return None;
        }
        None
    ...
    fn provide_definition(ctx: DocumentContext, _pos: Position) -> Vec<Location> {
        if !is_html(&ctx.language_id) {
            return vec![];
        }
        vec![]
    ...
    fn provide_references(ctx: DocumentContext, _pos: Position) -> Vec<Location> {
        if !is_html(&ctx.language_id) {
            return vec![];
        }
        vec![]
    ...
    fn provide_signature_help(ctx: DocumentContext, _pos: Position) -> Option<SignatureHelpResult> {
        if !is_html(&ctx.language_id) {
            return None;
        }
        None
    ...
    fn provide_inlay_hints(ctx: DocumentContext, _range: Range) -> Vec<InlayHint> {
        if !is_html(&ctx.language_id) {
            return vec![];
        }
        vec![]
...
fn is_html(lang: &str) -> bool {
    matches!(lang, "html" | "htm")
}

sidex_extension_sdk::export_extension!(HtmlLanguageExtension);
