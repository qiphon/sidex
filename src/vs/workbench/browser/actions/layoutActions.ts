...
const activityBarLeftIcon = registerIcon(
	...
	localize('activityBarLeft', 'Represents the activity bar in the left position')
);
const activityBarRightIcon = registerIcon(
	'activity-bar-right',
	Codicon.layoutActivitybarRight,
	localize('activityBarRight', 'Represents the activity bar in the right position')
);
...
const panelLeftOffIcon = registerIcon(
	...
	localize('panelLeftOff', 'Represents a side bar in the left position toggled off')
);
...
const panelRightOffIcon = registerIcon(
	...
	localize('panelRightOff', 'Represents side bar in the right position toggled off')
);
...
const panelAlignmentLeftIcon = registerIcon(
	...
	localize('panelBottomLeft', 'Represents the bottom panel alignment set to the left')
);
const panelAlignmentRightIcon = registerIcon(
	'panel-align-right',
	Codicon.layoutPanelRight,
	localize('panelBottomRight', 'Represents the bottom panel alignment set to the right')
);
const panelAlignmentCenterIcon = registerIcon(
	'panel-align-center',
	Codicon.layoutPanelCenter,
	localize('panelBottomCenter', 'Represents the bottom panel alignment set to the center')
);
const panelAlignmentJustifyIcon = registerIcon(
	'panel-align-justify',
	Codicon.layoutPanelJustify,
	localize('panelBottomJustify', 'Represents the bottom panel alignment set to justified')
);

const quickInputAlignmentTopIcon = registerIcon(
	'quickInputAlignmentTop',
	Codicon.arrowUp,
	localize('quickInputAlignmentTop', 'Represents quick input alignment set to the top')
);
const quickInputAlignmentCenterIcon = registerIcon(
	'quickInputAlignmentCenter',
	Codicon.circle,
	localize('quickInputAlignmentCenter', 'Represents quick input alignment set to the center')
);
...
const configureLayoutIcon = registerIcon(
	...
	Codicon.layout,
	localize('cofigureLayoutIcon', 'Icon represents workbench layout configuration.')
);
		...
		constructor() {
			super({
				...
				precondition: ContextKeyExpr.and(
					...
					ContextKeyExpr.or(PanelAlignmentContext.isEqualTo('center'), PanelPositionContext.notEqualsTo('bottom'))
				)
			});
		}
...
export class ShowMultipleEditorTabsAction extends AbstractSetShowTabsAction {
	...
	constructor() {
		...
		super(
			...
			precondition,
			localize2('showMultipleEditorTabsDescription', 'Show Tab Bar with multiple tabs')
		);
	}
}
...
export class ZenShowMultipleEditorTabsAction extends AbstractSetShowTabsAction {
	...
	constructor() {
		...
		super(
			...
			precondition,
			localize2('showMultipleEditorTabsZenModeDescription', 'Show Tab Bar in Zen Mode')
		);
	}
}
...
export class ZenShowSingleEditorTabAction extends AbstractSetShowTabsAction {
	...
	constructor() {
		...
		super(
			...
			precondition,
			localize2('showSingleEditorTabZenModeDescription', 'Show Tab Bar in Zen Mode with one Tab')
		);
	}
}
...
class MoveFocusedViewAction extends Action2 {
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!viewDescriptor?.canMoveView) {
			dialogService.error(
				localize('moveFocusedView.error.nonMovableView', 'The currently focused view is not movable.')
			);
			return;
		}
		...
		items.push(
			...pinnedViewlets
				...
				.map(viewletId => {
					return {
						id: viewletId,
						label: viewDescriptorService.getViewContainerModel(viewDescriptorService.getViewContainerById(viewletId)!)
							.title
					};
				})
		);
					...
					if (changeEvent.affectsSome(LayoutContextKeySet)) {
						...
						if (selectedItem) {
							quickPick.activeItems = quickPick.items.filter(
								item => (item as CustomizeLayoutItem).id === selectedItem?.id
							) as IQuickPickItem[];
						}
