...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		const updateMouseIndicatorColor = () => {
			mouseMarker.style.borderColor = Color.fromHex(
				configurationService.getValue<string>('screencastMode.mouseIndicatorColor')
			).toString();
		};
		...
		const updateMouseIndicatorSize = () => {
			mouseIndicatorSize = clamp(
				configurationService.getValue<number>('screencastMode.mouseIndicatorSize') || 20,
				20,
				100
			);
		...
		const updateKeyboardMarkerTimeout = () => {
			keyboardMarkerTimeout = clamp(
				configurationService.getValue<number>('screencastMode.keyboardOverlayTimeout') || 800,
				500,
				5000
			);
		};
		...
		function registerWindowListeners(window: Window, windowDisposables: DisposableStore): void {
			...
			listeners.add(
				listeners.add(new DomEmitter(window, 'compositionstart', true)).event(e => onCompositionStart.fire(e))
			);
			listeners.add(
				listeners.add(new DomEmitter(window, 'compositionupdate', true)).event(e => onCompositionUpdate.fire(e))
			);
		...
		const clearKeyboardScheduler = disposables.add(
		...
		disposables.add(
			onKeyDown.event(e => {
				...
				const commandDetails =
					this._isKbFound(shortcut) && shortcut.commandId ? this.getCommandDetails(shortcut.commandId) : undefined;
	...
	private getCommandDetails(commandId: string): { title: string; category?: string } | undefined {
		...
		if (fromCommandsRegistry?.metadata?.description) {
			return {
				title:
					typeof fromCommandsRegistry.metadata.description === 'string'
...
class LogStorageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2(
				{
					key: 'logStorage',
					comment: ['A developer only action to log the contents of the storage for the current window.']
				},
	...
	run(accessor: ServicesAccessor): void {
		...
		dialogService.info(
			localize('storageLogDialogMessage', 'The storage database contents have been logged to the developer tools.'),
			localize('storageLogDialogDetails', 'Open developer tools from the menu and select the Console tab.')
		);
	}
}
...
const DisposablesSnapshotStateContext = new RawContextKey<'started' | 'pending' | 'stopped'>(
	'dirtyWorkingCopies',
	'stopped'
);
...
class PolicyDiagnosticsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (policyConfigurations.size > 0) {
			...
			appliedPolicy.sort(
				(a, b) => getPolicySource(a.name).localeCompare(getPolicySource(b.name)) || a.name.localeCompare(b.name)
			);
