...
export class ResourcesDropHandler {
	...
	async handleDrop(
		...
		for (const { resource } of editors) {
			if (resource) {
				const handled = await this.instantiationService.invokeFunction(accessor =>
					dndRegistry.handleResourceDrop(resource, accessor)
				);
				if (handled) {
					return;
				}
...
export function fillEditorsDragData(
	...
	const resources = coalesce(
		resourcesOrEditors.map((resourceOrEditor): IResourceStat | undefined => {
		...
		if (firstFile) {
			...
			if (firstFileUri.scheme === Schemas.file) {
				event.dataTransfer.setData(
					...
					[Mimes.binary, basename(firstFile.resource), firstFileUri.toString()].join(':')
				);
			}
		}
	}
	...
	const draggedDirectories: URI[] = fileSystemResources
	...
	if (draggedEditors.length || draggedDirectories.length) {
		...
		for (const editor of draggedEditors) {
			if (editor.resource) {
				uriListEntries.push(
					editor.options?.selection ? withSelection(editor.resource, editor.options.selection) : editor.resource
				);
...
class GlobalWindowDraggedOverTracker extends Disposable {
	...
	private readonly broadcaster = this._register(
		new BroadcastDataChannel<boolean>(GlobalWindowDraggedOverTracker.CHANNEL_NAME)
	);
		...
		this._register(
			Event.runAndSubscribe(
				...
				({ window, disposables }) => {
					...
					disposables.add(
						addDisposableListener(window, EventType.DRAG_LEAVE, () => this.clearDraggedOver(false), true)
					);
				},
