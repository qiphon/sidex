		...
		return this.instantiationService.createInstance(
			...
			this.location,
			{
				...
				activityHoverOptions: {
					position: () =>
						this.layoutService.getSideBarPosition() === Position.LEFT ? HoverPosition.RIGHT : HoverPosition.LEFT
				},
...
export class ActivityBarCompositeBar extends PaneCompositeBar {
	...
	private registerKeyboardNavigationListeners(): void {
		...
		if (this.menuBarContainer) {
			this.keyboardNavigationDisposables.add(
				addDisposableListener(this.menuBarContainer, EventType.KEY_DOWN, e => {
	...
	getActivityBarContextMenuActions(): IAction[] {
		...
		const actions: IAction[] = [
			new SubmenuAction(
				'workbench.action.activityBar.position',
		...
		if (activityBarPosition === ActivityBarPosition.DEFAULT) {
			...
			const sizeActions = [
				toAction({
					...
					label: localize('activityBarSizeDefault', 'Default'),
					checked: !isCompact,
				...
				toAction({
					...
					label: localize('activityBarSizeCompact', 'Compact'),
					checked: isCompact,
			...
			actions.push(
				new SubmenuAction(
					'workbench.action.activityBar.size',
					localize('activity bar size', 'Activity Bar Size'),
					sizeActions
				)
			);
		}
		...
		if (this.part === Parts.SIDEBAR_PART) {
			actions.push(
				toAction({
					...
					run: () =>
						this.instantiationService.invokeFunction(accessor => new ToggleSidebarVisibilityAction().run(accessor))
				})
			);
		}
