	...
	return ExtensionsRegistry.registerExtensionPoint<IThemeExtensionPoint[]>({
		...
		jsonSchema: {
			...
			items: {
				type: 'object',
				defaultSnippets: [
					{ body: { id: '${1:id}', label: '${2:label}', path: './fileicons/${3:id}-icon-theme.json' } }
				],
				properties: {
					id: {
						description: nls.localize(
	...
	return ExtensionsRegistry.registerExtensionPoint<IThemeExtensionPoint[]>({
		...
		jsonSchema: {
			...
			items: {
				type: 'object',
				defaultSnippets: [
					{ body: { id: '${1:id}', label: '${2:label}', path: './producticons/${3:id}-product-icon-theme.json' } }
				],
				properties: {
					id: {
						description: nls.localize(
...
class ThemeDataRenderer extends Disposable implements IExtensionFeatureMarkdownRenderer {
	...
	shouldRender(manifest: IExtensionManifest): boolean {
		return (
			!!manifest.contributes?.themes || !!manifest.contributes?.iconThemes || !!manifest.contributes?.productIconThemes
		);
	}
