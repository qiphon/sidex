...
function _loadProductIconThemeDocument(
	...
	return fileService.readExtensionResource(location).then(content => {
		...
		if (parseErrors.length > 0) {
			...
			return Promise.reject(
				new Error(nls.localize('error.invalidformat', 'Invalid format for product icons theme file: Object expected.'))
			);
		...
		for (const font of contentValue.fonts) {
			...
			if (isString(fontId) && fontId.match(fontIdRegex)) {
				...
				if (isString(font.weight) && font.weight.match(fontWeightRegex)) {
					...
					warnings.push(
						nls.localize('error.fontWeight', "Invalid font weight in font '{0}'. Ignoring setting.", font.id)
					);
				}
				...
				if (isString(font.style) && font.style.match(fontStyleRegex)) {
					...
					warnings.push(
						nls.localize('error.fontStyle', "Invalid font style in font '{0}'. Ignoring setting.", font.id)
					);
				}
				...
				if (Array.isArray(font.src)) {
					for (const s of font.src) {
						if (isString(s.path) && isString(s.format) && s.format.match(fontFormatRegex)) {
							...
							warnings.push(
								nls.localize('error.fontSrc', "Invalid font source in font '{0}'. Ignoring source.", font.id)
							);
						}
					}
				}
				if (sanitizedSrc.length) {
					...
					warnings.push(
						nls.localize('error.noFontSrc', "No valid font source in font '{0}'. Ignoring font definition.", font.id)
					);
				}
			} else {
				warnings.push(
					nls.localize('error.fontId', "Missing or invalid font id '{0}'. Skipping font definition.", font.id)
				);
			}
		}
		...
		for (const iconId in contentValue.iconDefinitions) {
			...
			if (isString(definition.fontCharacter)) {
				...
				warnings.push(
					nls.localize('error.icon.fontCharacter', "Skipping icon definition '{0}': Needs to be defined", iconId)
				);
			}
		}
