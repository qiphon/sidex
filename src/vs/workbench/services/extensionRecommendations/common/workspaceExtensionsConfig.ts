...
export class WorkspaceExtensionsConfigService extends Disposable implements IWorkspaceExtensionsConfigService {
	...
	constructor(
		...
		this._register(
			fileService.onDidFilesChange(e => {
				...
				if (
					(workspace.configuration && e.affects(workspace.configuration)) ||
	...
	async getUnwantedRecommendations(): Promise<string[]> {
		const configs = await this.getExtensionsConfigs();
		return distinct(
			configs.flatMap(c => (c.unwantedRecommendations ? c.unwantedRecommendations.map(c => c.toLowerCase()) : []))
		);
	}
	...
	async toggleRecommendation(extensionId: string): Promise<void> {
		...
		const recommendedWorksapceFolders = workspace.folders.filter(workspaceFolder =>
		...
		const workspaceOrFolders = isRecommended
			? await this.pickWorkspaceOrFolders(
					...
					localize('select for remove', 'Remove extension recommendation from')
				)
			: await this.pickWorkspaceOrFolders(
					workspace.folders,
					workspace.configuration ? workspace : undefined,
					localize('select for add', 'Add extension recommendation to')
				);
	...
	async toggleUnwantedRecommendation(extensionId: string): Promise<void> {
		...
		const unWantedWorksapceFolders = workspace.folders.filter(workspaceFolder =>
			workspaceFolderExtensionsConfigContents
				.get(workspaceFolder.uri)
	...
	private async pickWorkspaceOrFolders(
		...
		const folderPicks: (
			| (IQuickPickItem & { workspaceOrFolder: IWorkspace | IWorkspaceFolder })
			| IQuickPickSeparator
	...
	private async resolveWorkspaceExtensionConfig(
		...
		try {
			...
			const extensionsConfigContent = <IExtensionsConfigContent | undefined>(
	...
	private parseExtensionConfig(extensionsConfigContent: IExtensionsConfigContent): IExtensionsConfigContent {
		return {
			...
			unwantedRecommendations: distinct(
				(extensionsConfigContent.unwantedRecommendations || []).map(e => e.toLowerCase())
			)
		};
	}
