		...
		resourceLabelFormattersExtPoint.setHandler((extensions, delta) => {
			for (const added of delta.added) {
				for (const untrustedFormatter of added.value) {
					...
					if (
						!isProposedApiEnabled(added.description, 'contribLabelFormatterWorkspaceTooltip') &&
...
export class LabelService extends Disposable implements ILabelService {
	...
	constructor(
		...
		this.os = OS;
		this.userHome =
			pathService.defaultUriScheme === Schemas.file ? this.pathService.userHome({ preferLocal: true }) : undefined;
	...
	findFormatting(resource: URI): ResourceLabelFormatting | undefined {
		...
		for (const formatter of this.formatters) {
			if (formatter.scheme === resource.scheme) {
				...
				if (
					match(formatter.authority, resource.authority, { ignoreCase: true }) &&
	...
	getUriLabel(
		resource: URI,
		options: { relative?: boolean; noPrefix?: boolean; separator?: '/' | '\\'; appendWorkspaceSuffix?: boolean } = {}
	): string {
	...
	getWorkspaceLabel(
		workspace: IWorkspace | IWorkspaceIdentifier | ISingleFolderWorkspaceIdentifier | URI,
	...
	private appendWorkspaceSuffix(label: string, uri: URI): string {
		...
		const suffix =
			formatting && typeof formatting.workspaceSuffix === 'string' ? formatting.workspaceSuffix : undefined;
