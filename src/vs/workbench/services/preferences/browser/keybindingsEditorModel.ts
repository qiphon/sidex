...
export class KeybindingsEditorModel extends EditorModel {
	...
	override async resolve(actionLabels = new Map<string, string>()): Promise<void> {
		...
		for (const keybinding of this.keybindingsService.getKeybindings()) {
			if (keybinding.command) {
				...
				this._keybindingItemsSortedByPrecedence.push(
					KeybindingsEditorModel.toKeybindingEntry(keybinding.command, keybinding, actionLabels, extensions)
				);
		...
		for (const command of getAllUnboundCommands(boundCommands)) {
			...
			this._keybindingItemsSortedByPrecedence.push(
				KeybindingsEditorModel.toKeybindingEntry(command, keybindingItem, actionLabels, extensions)
			);
		}
		this._keybindingItemsSortedByPrecedence = distinct(this._keybindingItemsSortedByPrecedence, keybindingItem =>
			KeybindingsEditorModel.getId(keybindingItem)
		);
	...
	private static getId(keybindingItem: IKeybindingItem): string {
		return (
			...
			keybindingItem.when +
			(isString(keybindingItem.source) ? keybindingItem.source : keybindingItem.source.identifier.value)
		);
	}
...
class KeybindingItemMatches {
	...
	private matchesKeybinding(
		...
		if (
			(userSettingsLabel && strings.compareIgnoreCase(searchValue, userSettingsLabel) === 0) ||
