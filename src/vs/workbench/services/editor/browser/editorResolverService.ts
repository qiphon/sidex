...
export class EditorResolverService extends Disposable implements IEditorResolverService {
	...
	constructor(
		...
		this.cache = new Set<string>(
			JSON.parse(
				this.storageService.get(EditorResolverService.cacheStorageID, StorageScope.PROFILE, JSON.stringify([]))
			)
		);
	...
	private resolveUntypedInputAndGroup(
		...
		| Promise<[IUntypedEditorInput, IEditorGroup, EditorActivation | undefined] | undefined>
	...
	async resolveEditor(
		...
		if (
			selectedEditor.editorFactoryObject.createDiffEditorInput === undefined &&
	...
	getAllUserAssociations(): EditorAssociations {
		const inspectedEditorAssociations =
			this.configurationService.inspect<{ [fileNamePattern: string]: string }>(editorsAssociationsSettingId) || {};
	...
	private findMatchingEditors(resource: URI): RegisteredEditor[] {
		...
		for (const [key, editors] of this._flattenedEditors) {
			for (const editor of editors) {
				...
				if (
					(foundInSettings && editor.editorInfo.priority !== RegisteredEditorPriority.exclusive) ||
		...
		return matchingEditors.sort((a, b) => {
			...
			if (
				priorityToRank(b.editorInfo.priority) === priorityToRank(a.editorInfo.priority) &&
	...
	private getEditor(
		...
		if (possibleEditors.length === 0) {
			return {
				editor:
					associationsFromSetting[0] && minPriority !== RegisteredEditorPriority.exclusive
		...
		const selectedViewType =
			possibleEditors[0].editorInfo.priority === RegisteredEditorPriority.exclusive
	...
	private cacheEditors() {
		...
		for (const [globPattern, contribPoint] of this._flattenedEditors) {
			const nonOptional = !!contribPoint.find(
				c =>
					c.editorInfo.priority !== RegisteredEditorPriority.option && c.editorInfo.id !== DEFAULT_EDITOR_ASSOCIATION.id
			);
