	...
	implements IProfileAwareExtensionManagementService
{
	...
	private readonly _onDidChangeProfile = this._register(
		new Emitter<{ readonly added: ILocalExtension[]; readonly removed: ILocalExtension[] }>()
	);
	...
	async install(location: URI, options: InstallOptions = {}): Promise<ILocalExtension> {
		...
		throw toExtensionManagementError(
			new Error(`Unknown error while installing extension ${getGalleryExtensionId(manifest.publisher, manifest.name)}`)
		);
	}
	...
	async installExtensionsFromProfile(
		...
		const extensionsToInstall = (await this.webExtensionsScannerService.scanUserExtensions(fromProfileLocation)).filter(
