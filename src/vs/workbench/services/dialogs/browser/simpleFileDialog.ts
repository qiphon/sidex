		...
		return accessor => {
			...
			if (activeEditorPane) {
				return editorService.save(
					{ groupId: activeEditorPane.group.id, editor: activeEditorPane.input },
					{ saveAs: true, availableFileSystems: [Schemas.file], reason: SaveReason.EXPLICIT }
				);
			}
...
export class SimpleFileDialog extends Disposable implements ISimpleFileDialog {
	...
	public async showOpenDialog(options: IOpenDialogOptions = {}): Promise<URI[] | undefined> {
		...
		this.scopedPathPrefix =
			options.defaultUri && this.scopedAuthority ? this.computeScopedPathPrefix(options.defaultUri) : '';
	...
	private remoteUriFrom(path: string, hintUri?: URI): URI {
		...
		const uri: URI =
			this.scheme === Schemas.file
				? URI.file(path)
				: URI.from({ scheme: this.scheme, path, query: hintUri?.query, fragment: hintUri?.fragment });
	...
	protected getUserHome(trueHome = false): Promise<URI> {
		...
		if (this.scopedAuthority) {
			return Promise.resolve(
				URI.from({ scheme: this.scheme, authority: this.scopedAuthority, path: this.scopedPathPrefix + '/' })
			);
		}
		return trueHome
	...
	private async pickResource(isSave: boolean = false): Promise<URI[] | URI | undefined> {
		...
		return new Promise<URI[] | URI | undefined>(resolve => {
			...
			this.filePickBox.okLabel =
				typeof this.options.openLabel === 'string' ? this.options.openLabel : this.options.openLabel?.withoutMnemonic;
			if (
				this.scheme !== Schemas.file &&
				this.options &&
	...
	private isValueChangeFromUser(): boolean {
		if (
			equalsIgnoreCase(
				...
				this.pathAppend(this.currentFolder, this.userEnteredPathSegment + this.autoCompletePathSegment)
			)
		) {
			return false;
		}
	...
	private async tryUpdateItems(value: string, valueUri: URI, reset: boolean = false): Promise<UpdateResult> {
		if (value.length > 0 && (value[0] === '~' || this.canTildaEscapeHatch(value))) {
			...
			if (
				(!newFolderIsOldFolder && (this.endsWithSlash(value) || newFolderIsParent || newFolderIsUnrelated)) ||
				reset
			) {
				let stat: IFileStatWithPartialMetadata | undefined;
				try {
				...
				if (stat?.isDirectory && resources.basename(valueUri) !== '.' && this.endsWithSlash(value)) {
					...
					if (
						!resources.extUriIgnorePathCase.isEqual(currentFolderWithoutSep, inputUriDirnameWithoutSep) &&
	...
	private setActiveItems(value: string) {
		...
		if (pathsEqual) {
			...
			if (!hasMatch) {
				const userBasename =
					inputBasename.length >= 2 ? userPath.substring(userPath.length - inputBasename.length + 2) : '';
