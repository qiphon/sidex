...
namespace schema {
	...
	export function isValidCommand(command: IUserFriendlyCommand, collector: ExtensionMessageCollector): boolean {
		...
		if (command.enablement && typeof command.enablement !== 'string') {
			collector.error(
				localize('optstring', 'property `{0}` can be omitted or must be of type `string`', 'precondition')
			);
			return false;
		}
	...
	function isValidIcon(icon: IUserFriendlyIcon | undefined, collector: ExtensionMessageCollector): boolean {
		...
		collector.error(
			localize('opticon', 'property `icon` can be omitted or must be either a string or a literal like `{dark, light}`')
		);
		return false;
	}
	...
	function isValidLocalizedString(
		...
		if (typeof localized === 'undefined') {
			...
			collector.error(
				localize('requirestring', 'property `{0}` is mandatory and must be of type `string`', propertyName)
			);
			return false;
		} else if (
	...
	for (const extension of extensions) {
		...
		for (const [, submenuInfo] of Object.entries(value)) {
			...
			if (_submenus.has(submenuInfo.id)) {
				collector.info(
					localize('submenuId.duplicate.id', 'The `{0}` submenu was already previously registered.', submenuInfo.id)
				);
				continue;
			}
			...
			for (const menuItem of entry[1]) {
				...
				if (schema.isMenuItem(menuItem)) {
					...
					if (menuItem.command === menuItem.alt) {
						collector.info(
							localize('dupe.command', 'Menu item references the same command as default and alt-command')
						);
					}
