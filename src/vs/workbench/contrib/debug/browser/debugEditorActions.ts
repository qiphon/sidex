...
class StepIntoTargetsAction extends EditorAction {
	...
	public static readonly LABEL = nls.localize(
		{
			key: 'stepIntoTargets',
			comment: ['Step Into Targets lets the user step into an exact function he or she is interested in.']
		},
		'Step Into Target'
	);
	...
	async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
		...
		if (selection) {
			...
			const best =
				positionalTargets.find(t => t.end && needle.isBefore(t.end) && t.start.isBeforeOrEqual(needle)) ||
		...
		contextMenuService.showContextMenu({
			...
			getActions: () => {
				return targets.map(t =>
					toAction({
						id: `stepIntoTarget:${t.id}`,
						label: t.label,
						enabled: true,
						run: () => session.stepIn(frame.thread.threadId, t.id)
					})
				);
			}
		});
