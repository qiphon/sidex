...
export class DebugToolBar extends Themable implements IWorkbenchContribution {
				...
				actionViewItemProvider: (action: IAction, options: IBaseActionViewItemOptions) => {
					if (action.id === FOCUS_SESSION_ID) {
						...
						const item = this.instantiationService.invokeFunction(accessor =>
							createDisconnectMenuItemAction(action as MenuItemAction, this.stopActionViewItemDisposables, accessor, {
	...
	private registerListeners(): void {
		...
		this._register(
			this.actionBar.actionRunner.onDidRun((e: IRunEvent) => {
				...
				this.telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>(
					...
					{ id: e.action.id, from: 'debugActionsWidget' }
				);
			})
		);

		this._register(
			dom.addDisposableGenericMouseUpListener(this.dragArea, (event: MouseEvent) => {
		...
		this._register(
			dom.addDisposableGenericMouseDownListener(this.dragArea, (e: MouseEvent) => {
...
const registerDebugToolBarItem = (
	id: string,
