...
export class TauriDebugContribution extends Disposable {
	...
	private async scanExtensionDebuggers(): Promise<void> {
		const adapters = await this.tauriService.scanExtensionDebuggers();
		if (adapters.length > 0) {
			// Update the adapter manager schema after new debuggers are discovered
			// This triggers launch.json IntelliSense updates
			const adapterManager = this.debugService.getAdapterManager();
			if ('onDidRegisterDebugger' in adapterManager) {
				// The schema update happens automatically in the AdapterManager
				// when debuggers are registered via the extension point
			}
		}
	}
	...
	private registerAdapterFactory(): void {
		const adapterManager = this.debugService.getAdapterManager();
		if (adapterManager instanceof AdapterManager) {
			// Register a descriptor factory for all types that aren't already handled
			// by extension-provided factories. We use a wildcard approach: the factory
			// checks the Rust registry for each type at session creation time.
			this._register(
				adapterManager.registerDebugAdapterDescriptorFactory(this.descriptorFactory)
			);
		}
	...
	async installAdditionalDebuggers(languageLabel?: string): Promise<void> {
		...
		if (items.length === 0) {
			await this.dialogService.info(
				nls.localize('noDebugAdaptersFound', 'No Debug Adapters Found'),
				nls.localize(
					'noDebugAdaptersFoundDetail',
					"No debug adapter extensions were found for '{0}' in the marketplace.",
					query
				)
			);
			return;
