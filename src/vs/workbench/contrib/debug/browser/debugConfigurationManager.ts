...
export class ConfigurationManager implements IConfigurationManager {
	...
	async getDynamicProviders(): Promise<
		{
			...
			pick: () => Promise<{ launch: ILaunch; config: IConfig; label: string } | undefined>;
		}[]
	> {
	...
	async getDynamicConfigurationsByType(
		...
		this.getLaunches().forEach(launch => {
			if (provider) {
				picks.push(
					provider.provideDebugConfigurations!(launch.workspace?.uri, token).then(configurations =>
	...
	private setCompoundSchemaValues(): void {
		const compoundConfigurationsSchema = (<IJSONSchema>launchSchema.properties!['compounds'].items).properties![
			'configurations'
		];
...
abstract class AbstractLaunch implements ILaunch {
	...
	abstract openConfigFile(
		options: { preserveFocus: boolean; type?: string | undefined; suppressInitialConfigs?: boolean | undefined },
	...
	getConfigurationNames(ignoreCompoundsAndPresentation = false): string[] {
		...
		if (!config || (!Array.isArray(config.configurations) && !Array.isArray(config.compounds))) {
			...
			const resolved = configurations.map(c =>
				isDebugConfig(c) ? getEffectiveConfigForPlatform(c, this.configurationManager.getTargetOperatingSystem()) : c
			);
	...
	getConfiguration(name: string): IConfig | undefined {
		...
		const effectiveConfiguration = getEffectiveConfigForPlatform(
			configuration,
			this.configurationManager.getTargetOperatingSystem()
		);
...
class Launch extends AbstractLaunch implements ILaunch {
	...
	protected getConfig(): IGlobalConfig | undefined {
		return this.configurationService.inspect<IGlobalConfig>('launch', { resource: this.workspace.uri })
	...
	async openConfigFile(
		...
		}: { preserveFocus: boolean; type?: string; suppressInitialConfigs?: boolean },
		token?: CancellationToken
	): Promise<{ editor: IEditorPane | null; created: boolean }> {
...
class WorkspaceLaunch extends AbstractLaunch implements ILaunch {
	...
	async openConfigFile(
		{ preserveFocus, type, useInitialConfigs }: { preserveFocus: boolean; type?: string; useInitialConfigs?: boolean },
		token?: CancellationToken
	): Promise<{ editor: IEditorPane | null; created: boolean }> {
