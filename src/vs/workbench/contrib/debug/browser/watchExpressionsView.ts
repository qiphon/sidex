...
export class WatchExpressionsView extends ViewPane implements IDebugViewWithVariables {
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this.tree = this.instantiationService.createInstance(
			WorkbenchAsyncDataTree<IDebugService | IExpression, IExpression, FuzzyScore>,
			'WatchExpressions',
			treeContainer,
	...
	private onMouseDblClick(e: ITreeMouseEvent<IExpression>): void {
		...
		if (
			(element instanceof Expression && element !== selectedExpression?.expression) ||
	...
	private async onContextMenu(e: ITreeContextMenuEvent<IExpression>): Promise<void> {
		...
		const contextKeyService =
			element &&
			(await getContextForWatchExpressionMenuWithDataAccess(
 ...
 */
async function getContextForWatchExpressionMenuWithDataAccess(
	parentContext: IContextKeyService,
	expression: IExpression,
...
registerAction2(
	class CopyExpression extends ViewAction<WatchExpressionsView> {
		constructor() {
			super({
