...
export class BreakpointsView extends ViewPane {
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this._register(
			this.tree.onMouseMiddleClick(async ({ element }) => {
				if (element instanceof Breakpoint) {
		...
		this._register(
			this.tree.onDidOpen(async e => {
				...
				if (
					...
					element !== this.inputBoxData?.breakpoint
				) {
					// double click
				...
				hoverDelegate: {
					showHover: (options, focus?) =>
						this.hoverService.showInstantHover(
							{ content: options.content, target: this.hintContainer!.element },
							focus
						),
	...
	private onTreeContextMenu(e: ITreeContextMenuEvent<BreakpointTreeElement | null>): void {
		...
		this.breakpointHasMultipleModes.set(
			this.debugService.getModel().getBreakpointModes(getModeKindForBreakpoint(element as IBreakpoint)).length > 1
		);
	...
	private updateSize(): void {
		...
		this.minimumBodySize =
			this.orientation === Orientation.VERTICAL
				? Math.min(MAX_VISIBLE_BREAKPOINTS * rowHeight, this.tree.contentHeight)
				: 170;
		this.maximumBodySize =
			this.orientation === Orientation.VERTICAL && containerModel.visibleViewDescriptors.length > 1
				? this.tree.contentHeight
...
> {
	...
	renderElement(
		node: ITreeNode<BreakpointsFolderItem, void>,
		_index: number,
		data: IBreakpointsFolderTemplateData
	): void {
		...
		const removeAction = data.elementDisposables.add(
			new Action(
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<BreakpointsFolderItem>, void>,
		_index: number,
		data: IBreakpointsFolderTemplateData
	): void {
	...
	disposeElement(
		element: ITreeNode<BreakpointsFolderItem, void>,
		index: number,
		templateData: IBreakpointsFolderTemplateData
	): void {
...
class BreakpointsRenderer implements ICompressibleTreeRenderer<IBreakpoint, void, IBreakpointTemplateData> {
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<IBreakpoint>, void>,
		index: number,
	...
	private renderBreakpointCommon(breakpoint: IBreakpoint, data: IBreakpointTemplateData): void {
		...
		const { primary } = getActionBarActions(
			this.menu.getActions({ arg: breakpoint, shouldForwardArgs: true }),
			'inline'
		);
		data.actionBar.clear();
	...
	private renderBreakpointLineLabel(breakpoint: IBreakpoint, data: IBreakpointTemplateData): void {
		...
		this.textModelService
			.createModelReference(breakpoint.uri)
			.then(reference => {
...
> {
	...
	renderElement(
		node: ITreeNode<IExceptionBreakpoint, void>,
		index: number,
	...
	private renderExceptionBreakpoint(
		...
		const exceptionBreakpointtitle = exceptionBreakpoint.verified
			...
			: exceptionBreakpoint.message || localize('unverifiedExceptionBreakpoint', 'Unverified Exception Breakpoint');
		data.elementDisposables.add(
			this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), data.breakpoint, exceptionBreakpointtitle)
		);
		...
		data.elementDisposables.add(
			this.hoverService.setupManagedHover(
				getDefaultHoverDelegate('mouse'),
				data.condition,
				localize('expressionCondition', 'Expression condition: {0}', exceptionBreakpoint.condition)
			)
		);
		...
		const { primary } = getActionBarActions(
			this.menu.getActions({ arg: exceptionBreakpoint, shouldForwardArgs: true }),
			'inline'
		);
		data.actionBar.clear();
		data.actionBar.push(primary, { icon: true, label: false });
...
> {
	...
	renderElement(
		node: ITreeNode<FunctionBreakpoint, void>,
		_index: number,
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<FunctionBreakpoint>, void>,
		_index: number,
		data: IFunctionBreakpointTemplateData
	): void {
	...
	private renderFunctionBreakpoint(
		...
		if (session && !session.capabilities.supportsFunctionBreakpoints) {
			data.elementDisposables.add(
				this.hoverService.setupManagedHover(
					getDefaultHoverDelegate('mouse'),
					data.breakpoint,
					localize('functionBreakpointsNotSupported', 'Function breakpoints are not supported by this debug type')
				)
			);
		}
		...
		const { primary } = getActionBarActions(
			this.menu.getActions({ arg: functionBreakpoint, shouldForwardArgs: true }),
			'inline'
		);
		data.actionBar.clear();
		data.actionBar.push(primary, { icon: true, label: false });
...
class DataBreakpointsRenderer implements ICompressibleTreeRenderer<DataBreakpoint, void, IDataBreakpointTemplateData> {
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<DataBreakpoint>, void>,
		_index: number,
	...
	private renderDataBreakpoint(dataBreakpoint: DataBreakpoint, data: IDataBreakpointTemplateData): void {
		...
		if (session && !session.capabilities.supportsDataBreakpoints) {
			data.elementDisposables.add(
				this.hoverService.setupManagedHover(
					getDefaultHoverDelegate('mouse'),
					data.breakpoint,
					localize('dataBreakpointsNotSupported', 'Data breakpoints are not supported by this debug type')
				)
			);
		}
		...
		const { primary } = getActionBarActions(
			this.menu.getActions({ arg: dataBreakpoint, shouldForwardArgs: true }),
			'inline'
		);
		data.actionBar.clear();
		data.actionBar.push(primary, { icon: true, label: false });
	...
	disposeElement(
		node: ITreeNode<DataBreakpoint, void>,
		index: number,
...
> {
	...
	renderElement(
		node: ITreeNode<IInstructionBreakpoint, void>,
		index: number,
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<IInstructionBreakpoint>, void>,
		index: number,
		data: IInstructionBreakpointTemplateData
	): void {
	...
	private renderInstructionBreakpoint(
		...
		data.elementDisposables.add(
			this.hoverService.setupManagedHover(
				getDefaultHoverDelegate('mouse'),
				data.name,
				localize('debug.decimal.address', 'Decimal Address: {0}', breakpoint.address.toString())
			)
		);
		data.checkbox.checked = breakpoint.enabled;
...
> {
	...
	renderTemplate(container: HTMLElement): IFunctionBreakpointInputTemplateData {
		...
		toDispose.add(
			dom.addStandardDisposableListener(inputBox.inputElement, 'keydown', (e: IKeyboardEvent) => {
	...
	disposeElement(
		node: ITreeNode<IFunctionBreakpoint, void>,
		index: number,
...
> {
	...
	disposeElement(
		node: ITreeNode<IDataBreakpoint, void>,
		index: number,
...
> {
	...
	renderElement(
		node: ITreeNode<ExceptionBreakpoint, void>,
		_index: number,
	...
	getAriaLabel(element: BreakpointTreeElement): string | null {
		if (element instanceof BreakpointsFolderItem) {
			return localize(
				...
				resources.basenameOrAuthority(element.uri),
...
export function getBreakpointMessageAndIcon(
...
): { message?: string; icon: ThemeIcon; showAdapterUnverifiedMessage?: boolean } {
	...
	const message = breakpoint.message
		? breakpoint.message
...
abstract class MemoryBreakpointAction extends Action2 {
	async run(accessor: ServicesAccessor, existingBreakpoint?: IDataBreakpoint): Promise<void> {
		...
		try {
			...
			notifications.error(
				localize('dataBreakpointError', 'Failed to set data breakpoint at {0}: {1}', range.address, e.message)
			);
		}
	...
	private parseAddress(
		...
		if (sign === '-') {
			if (start > end) {
				return {
					error: localize('dataBreakpointAddrOrder', 'End ({1}) should be greater than Start ({0})', startStr, endStr)
				};
			}
		...
		constructor() {
			super({
				...
				title: {
					...
					mnemonicTitle: localize(
						{ key: 'miDataBreakpoint', comment: ['&& denotes a mnemonic'] },
						'&&Data Breakpoint...'
					)
				},
				f1: true,
...
registerAction2(
	class extends ViewAction<BreakpointsView> {
		...
		async runInView(
			...
			breakpoint: ExceptionBreakpoint | Breakpoint | FunctionBreakpoint | DataBreakpoint
		): Promise<void> {
			const debugService = accessor.get(IDebugService);
