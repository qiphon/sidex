...
export class DebugHoverWidget implements IContentWidget {
	...
	private create(): void {
		...
		this.tree = this.instantiationService.createInstance(
			WorkbenchAsyncDataTree<IExpression, IExpression, any>,
			'DebugHover',
			this.treeContainer,
	...
	private registerListeners(): void {
		this.toDispose.push(
			dom.addStandardDisposableListener(this.domNode, 'keydown', (e: IKeyboardEvent) => {
	...
	private async doShow(
		...
		if (!expression.hasChildren) {
			...
			store.add(
				this.expressionRenderer.renderValue(this.valueContainer, expression, {
					showChanged: false,
					colorize: true,
					hover: false,
					session
	...
	private layoutTree(): void {
		...
		if (this.showAtPosition) {
			...
			const hoveredCharTop =
				this.editor.getTopForLineNumber(this.showAtPosition.lineNumber, true) - this.editor.getScrollTop();
