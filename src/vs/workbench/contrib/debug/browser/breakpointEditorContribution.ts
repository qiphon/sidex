...
function getBreakpointDecorationOptions(
	...
	const renderInline =
		breakpoint.column &&
		(hasOtherBreakpointsOnLine || breakpoint.column > model.getLineFirstNonWhitespaceColumn(breakpoint.lineNumber));
	return {
...
): { range: Range; options: IModelDecorationOptions; breakpoint: IBreakpoint | undefined }[] {
	...
	private registerListeners(): void {
									...
									},
									{
										label: nls.localize(
											...
											enabled
												...
												: nls.localize({ key: 'enable', comment: ['&& denotes a mnemonic'] }, '&&Enable'),
											breakpointType
										),
		...
		this.toDispose.push(
			this.configurationService.onDidChangeConfiguration(async e => {
				if (
					e.affectsConfiguration('debug.showBreakpointsInOverviewRuler') ||
		...
		if (breakpoints.length === 1) {
			...
			actions.push(
				toAction({
					...
					label: breakpoints[0].enabled
						? nls.localize('disableBreakpoint', 'Disable {0}', breakpointType)
						: nls.localize('enableBreakpoint', 'Enable {0}', breakpointType),
					enabled: true,
					...
					sorted.map(bp =>
						toAction({
							...
							label: bp.column
								...
								: nls.localize('removeLineBreakpoint', 'Remove Line Breakpoint'),
							enabled: true,
						...
						toAction({
							...
							label: bp.enabled
								...
								: bp.column
									...
									: nls.localize('enableBreakpointOnLine', 'Enable Line Breakpoint'),
							enabled: true,
		...
		if (decorations) {
			for (const { options } of decorations) {
				...
				const hasSomeActionableCodicon =
					!(clz.includes('codicon-') || clz.startsWith('coverage-deco-')) ||
...
GutterActionsRegistry.registerGutterActionsGenerator(({ lineNumber, editor, accessor }, result) => {
	...
	const breakpointEditorContribution = editor.getContribution<IBreakpointEditorContribution>(
		BREAKPOINT_EDITOR_CONTRIBUTION_ID
	);
...
class InlineBreakpointWidget implements IContentWidget, IDisposable {
	...
	private create(cssClass: string | null | undefined): void {
		...
		this.toDispose.push(
			dom.addDisposableListener(this.domNode, dom.EventType.CONTEXT_MENU, e => {
	...
	private updateSize() {
		const lineHeight = this.range
			? this.editor.getLineHeightForPosition(this.range.getStartPosition())
...
const debugIconBreakpointDisabledForeground = registerColor(
	...
	'#848484',
	nls.localize('debugIcon.breakpointDisabledForeground', 'Icon color for disabled breakpoints.')
);
const debugIconBreakpointUnverifiedForeground = registerColor(
	'debugIcon.breakpointUnverifiedForeground',
	'#848484',
	nls.localize('debugIcon.breakpointUnverifiedForeground', 'Icon color for unverified breakpoints.')
);
...
const debugIconBreakpointStackframeForeground = registerColor(
	...
	'#89D185',
	nls.localize('debugIcon.breakpointStackframeForeground', 'Icon color for all breakpoint stack frames.')
);
