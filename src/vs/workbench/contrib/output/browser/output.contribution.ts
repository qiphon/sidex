...
class OutputContribution extends Disposable implements IWorkbenchContribution {
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						const extensionLogs: IOutputChannelDescriptor[] = [],
							logs: IOutputChannelDescriptor[] = [];
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						const entries: Array<IOutputChannelDescriptor> = outputService
					...
					async run(accessor: ServicesAccessor, args?: unknown): Promise<void> {
						...
						if (!entry) {
							...
							entry = <IQuickPickItem | undefined>(
								await quickInputService.pick(entries, { placeHolder: nls.localize('selectlogFile', 'Select Log File') })
							);
						}
						if (entry?.id) {
	...
	private registerLogLevelFilterActions(): void {
		...
		const registerLogLevel = (logLevel: LogLevel, toggled: ContextKeyExpression) => {
			this._register(
				registerAction2(
					class extends ViewAction<OutputViewPane> {
						constructor() {
							super({
			...
			registerAction2(
				class extends Action2 {
					...
					async run(
						...
						arg?: { outputPath?: URI; outputChannelIds?: string[] }
					): Promise<void> {
						const outputService = accessor.get(IOutputService);
						const quickInputService = accessor.get(IQuickInputService);
						const extensionLogs: IOutputChannelDescriptor[] = [],
							logs: IOutputChannelDescriptor[] = [],
							userLogs: IOutputChannelDescriptor[] = [];
						for (const channel of outputService.getChannelDescriptors()) {
							if (channel.log) {
								if (channel.extensionId) {
									extensionLogs.push(channel);
						...
						if (arg?.outputChannelIds) {
							...
							if (requestedIdsNormalized.includes('*')) {
								...
								selectedOutputChannels = candidates.filter(candidate =>
									requestedIdsNormalized.includes(candidate.id.toLowerCase())
								);
							}
						} else {
							selectedOutputChannels = await quickInputService.pick(entries, {
								placeHolder: nls.localize('selectlog', 'Select Log'),
								canPickMany: true
							});
						}
