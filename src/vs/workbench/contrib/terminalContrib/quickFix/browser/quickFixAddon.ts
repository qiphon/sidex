...
export class TerminalQuickFixAddon extends Disposable implements ITerminalAddon, ITerminalQuickFixAddon {
	...
	private _currentRenderContext:
		| { quickFixes: ITerminalAction[]; anchor: IAnchor; parentElement: HTMLElement }
		| undefined;
	...
	private readonly _onDidRequestRerunCommand = this._register(
		new Emitter<{ command: string; shouldExecute?: boolean }>()
	);
	readonly onDidRequestRerunCommand = this._onDidRequestRerunCommand.event;
	private readonly _onDidUpdateQuickFixes = this._register(
		new Emitter<{ command: ITerminalCommand; actions: ITerminalAction[] | undefined }>()
	);
	...
	constructor(
		...
		if (commandDetectionCapability) {
			...
			this._register(
				this._capabilities.onDidAddCommandDetectionCapability(() => {
					this._registerCommandHandlers();
				})
			);
		}
		this._register(
			this._quickFixService.onDidRegisterProvider(result =>
				this.registerCommandFinishedListener(convertToQuickFixOptions(result))
			)
		);
	...
	private _registerCommandHandlers(): void {
		...
		this._register(
			commandDetection.onCommandFinished(async command => await this._resolveQuickFixes(command, this._aliases))
		);
	}
	...
	private _registerQuickFixDecoration(): void {
		...
		const quickFixEnabled = this._configurationService.getValue<boolean>(
...
export async function getQuickFixesForCommand(
	...
	getResolvedFixes?: (
		...
		lines?: string[]
	) => Promise<SingleOrMany<ITerminalQuickFix> | undefined>
	...
	for (const options of quickFixOptions.values()) {
		for (const option of options) {
			if (
				(option.commandExitResult === 'success' && terminalCommand.exitCode !== 0) ||
			...
			if (option.type === 'resolved') {
				quickFixes = await (option as ITerminalQuickFixResolvedExtensionOptions).getQuickFixes(
					terminalCommand,
