			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor, args?: IOpenSettingsActionOptions) {
						...
						const workspaceFolder = await commandService.executeCommand<IWorkspaceFolder>(
				...
				class extends Action2 {
					...
					run(accessor: ServicesAccessor) {
						accessor
							...
							.openWorkspaceSettings({ jsonEditor: false, query: `@tag:${REQUIRE_TRUSTED_WORKSPACE_SETTING_TAG}` });
					}
				}
			)
		);
	...
	private updatePreferencesEditorMenuItem() {
		...
		if (
			...
			const when = ContextKeyExpr.and(
				ResourceContextKey.Resource.isEqualTo(this.preferencesService.workspaceSettingsResource!.toString()),
				...
				switch (settingsModel.configurationTarget) {
					case ConfigurationTarget.WORKSPACE:
						this.currentRenderer = this.disposables.add(
							this.instantiationService.createInstance(WorkspaceSettingsRenderer, this.editor, settingsModel)
						);
						break;
					default:
						this.currentRenderer = this.disposables.add(
							this.instantiationService.createInstance(UserSettingsRenderer, this.editor, settingsModel)
						);
						break;
				}
			}
