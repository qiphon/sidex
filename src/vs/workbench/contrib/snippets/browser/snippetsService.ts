...
export class SnippetsService implements ISnippetsService {
	...
	private _addSnippetFile(uri: URI, source: SnippetSource): IDisposable {
		...
		if (source === SnippetSource.User && ext === '.json') {
			...
			this._files.set(
				uri,
				new SnippetFile(source, uri, [langName], undefined, this._fileService, this._extensionResourceLoaderService)
			);
		} else if (ext === '.code-snippets') {
			this._files.set(
				uri,
				new SnippetFile(source, uri, undefined, undefined, this._fileService, this._extensionResourceLoaderService)
			);
		}
		return {
