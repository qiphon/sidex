...
export class SearchModelImpl extends Disposable implements ISearchModel {
	...
	get hasAIResults(): boolean {
		return (
			...
			(!!this.currentAICancelTokenSource && !this.currentAICancelTokenSource.token.isCancellationRequested)
		);
	}
	...
	get hasPlainResults(): boolean {
		return (
			...
			(!!this.currentCancelTokenSource && !this.currentCancelTokenSource.token.isCancellationRequested)
		);
	}
