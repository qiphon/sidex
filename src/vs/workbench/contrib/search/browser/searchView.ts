...
export class SearchView extends ViewPane {
	...
	constructor(
		...
		this.isTreeLayoutViewVisible =
			this.viewletState.view?.treeLayout ?? this.searchConfig.defaultViewMode === ViewMode.Tree;
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		this._register(
			dom.addDisposableListener(this.toggleQueryDetailsButton, dom.EventType.CLICK, e => {
				dom.EventHelper.stop(e);
				this.toggleQueryDetails(!this.accessibilityService.isScreenReaderOptimized());
			})
		);
		this._register(
			dom.addDisposableListener(this.toggleQueryDetailsButton, dom.EventType.KEY_UP, (e: KeyboardEvent) => {
		...
		this._register(
			dom.addDisposableListener(this.toggleQueryDetailsButton, dom.EventType.KEY_DOWN, (e: KeyboardEvent) => {
				const event = new StandardKeyboardEvent(e);
	...
	private createSearchWidget(container: HTMLElement): void {
		...
		const showReplace =
			typeof this.viewletState.view?.showReplace === 'boolean' ? this.viewletState.view.showReplace : true;
		...
		this._register(
			this.searchWidget.onSearchSubmit(options => {
				const shouldRenderAIResults =
					this.configurationService.getValue<ISearchConfigurationProperties>('search').searchView
						.semanticSearchBehavior;
		...
		this._register(
			this.searchWidget.getNotebookFilters().onDidChange(() => this.triggerQueryChange({ shouldKeepAIResults: true }))
		);
		...
		const updateHasReplacePatternKey = () =>
			this.hasReplacePatternKey.set(this.searchWidget.getReplaceValue().length > 0);
	...
	private createSearchResultsView(container: HTMLElement): void {
		...
		const updateHasSomeCollapsible = () =>
			this.toggleCollapseStateDelayer.trigger(() => this.hasSomeCollapsibleResultKey.set(this.hasSomeCollapsible()));
			...
			Event.any<any>(
				...
				if (this.tree.isDOMFocused()) {
					...
					this.fileMatchOrFolderMatchWithResourceFocus.set(
						isSearchTreeFileMatch(focus) || isSearchTreeFolderMatchWithResource(focus)
					);
	...
	private updateTextFromSelection({ allowUnselectedWord = true, allowSearchOnType = true }, editor?: IEditor): boolean {
		const seedSearchStringFromSelection =
			this.configurationService.getValue<IEditorOptions>('editor').find!.seedSearchStringFromSelection;
	...
	private async onSearchComplete(
		...
		Constants.SearchContext.AIResultsRequested.bindTo(this.contextKeyService).set(
			this.shouldShowAIResults() && !!aiResults
		);
		...
		if (
			...
			this.tree.isCollapsed(this.searchResult.aiTextSearchResult)
		) {
	...
	private doSearch(
		...
		this.progressService.withProgress(
			{ location: this.getProgressLocation(), delay: triggeredOnType ? 300 : 0 },
			_progress => {
				return new Promise<void>(resolve => (progressComplete = resolve));
			}
		);
	...
	private updateSearchResultCount(
		...
		this.tree.ariaLabel =
			resultMsg + nls.localize('forTerm', ' - Search: {0}', this.searchResult.query?.contentPattern.pattern ?? '');
			...
			if (disregardExcludesAndIgnores) {
				...
				const enableExcludesButton = this.messageDisposables.add(
					new SearchLinkButton(
						...
						this.hoverService,
						nls.localize('useExcludesAndIgnoreFilesDescription', 'Use Exclude Settings and Ignore Files')
					)
				);
	...
	private updateKeywordSuggestionUI(keyword: AISearchKeyword) {
		...
		if (this._cachedKeywords.length > 0) {
			...
			const button = this.messageDisposables.add(
				new SearchLinkButton(keyword.keyword, () => this.handleKeywordClick(keyword.keyword, index), this.hoverService)
			);
			...
			const button = this.messageDisposables.add(
				new SearchLinkButton(keyword.keyword, () => this.handleKeywordClick(keyword.keyword, 0), this.hoverService)
			);
	...
	private showEmptyStage(forceHideMessages = false): void {
		const showingCancelled =
			(this.messagesElement.firstChild?.textContent?.indexOf(SEARCH_CANCELLED_MESSAGE) ?? -1) > -1;
	...
	private shouldOpenInNotebookEditor(match: ISearchTreeMatch, uri: URI): boolean {
		...
		return (
			...
			(uri.scheme !== network.Schemas.untitled && this.notebookService.getContributedNotebookTypes(uri).length > 0)
		);
	}
	...
	async open(
		...
		const resource =
			resourceInput ??
			(isSearchTreeMatch(element) ? element.parent().resource : (<ISearchTreeFileMatch>element).resource);
	...
	getChildren(
		...
		if (isSearchResult(element)) {
			...
			if (
				...
				(!this.searchView.model.hasAIResults || !!this.searchView._pendingSemanticSearchPromise)
			) {
