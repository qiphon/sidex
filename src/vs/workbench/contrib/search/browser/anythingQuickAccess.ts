...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
	...
	private doGetPicks(
		...
		return {
			...
			additionalPicks: (async (): Promise<Picks<IAnythingQuickPickItem>> => {
				...
				const additionalPicksExcludes = new ResourceMap<boolean>(uri =>
	...
	private getEditorHistoryPicks(query: IPreparedQuery): Array<IAnythingQuickPickItem> {
		...
		return editorHistoryPicks.sort((editorA, editorB) =>
			compareItemsByFuzzyScore(editorA, editorB, query, false, editorHistoryScorerAccessor, this.pickState.scorerCache)
		);
	}
	...
	private createFileQueryCache(): FileQueryCacheState {
		return new FileQueryCacheState(
			cacheKey =>
				this.fileQueryBuilder.file(this.contextService.getWorkspace().folders, this.getFileQueryOptions({ cacheKey })),
	...
	private async doFileSearch(query: IPreparedQuery, token: CancellationToken): Promise<URI[]> {
		...
		const relativePathFileResultsMap = new ResourceMap<boolean>(uri =>
			this.uriIdentityService.extUri.getComparisonKey(uri)
		);
	...
	private async getFileSearchResults(query: IPreparedQuery, token: CancellationToken): Promise<URI[]> {
		...
		if (fileSearchResults.limitHit && query.values && query.values.length > 1) {
			...
			const existingFileSearchResultsMap = new ResourceMap<boolean>(uri =>
				this.uriIdentityService.extUri.getComparisonKey(uri)
			);
	...
	private getEditorSymbolPicks(
		...
		if (
			...
			(!this.fileService.hasProvider(activeGlobalResource) && activeGlobalResource.scheme !== Schemas.untitled)
		) {
	...
	private createAnythingPick(
		...
		if (isEditorInput(resourceOrEditor)) {
			...
			isDirty =
				this.workingCopyService.isDirty(resource) && !this.filesConfigurationService.hasShortAutoSaveDelay(resource);
			extraClasses = [];
		}
