...
export class DefaultFormatter extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._store.add(
			FormattingConflicts.setFormatterSelector((formatter, document, mode, kind) =>
	...
	private async _analyzeFormatter<T extends FormattingEditProvider>(
		...
		if (defaultFormatterId) {
			...
			const defaultFormatter = formatter.find(formatter =>
				ExtensionIdentifier.equals(formatter.extensionId, defaultFormatterId)
			);
			if (defaultFormatter) {
				// formatter available
	...
	private async _pickAndPersistDefaultFormatter<T extends FormattingEditProvider>(
		formatter: T[],
		document: ITextModel
	): Promise<T | undefined> {
	...
	private _updateStatus() {
		...
		this._analyzeFormatter(FormattingKind.File, formatter, document).then(result => {
			...
			this._languageStatusStore.add(
				CommandsRegistry.registerCommand(command.id, () => this._pickAndPersistDefaultFormatter(formatter, document))
			);
			this._languageStatusStore.add(
