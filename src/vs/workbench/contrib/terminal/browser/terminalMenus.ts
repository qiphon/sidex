	...
	for (const menuId of [MenuId.EditorTitle, MenuId.CompactWindowEditorTitle]) {
		...
		MenuRegistry.appendMenuItem(menuId, {
			...
			when: ContextKeyExpr.and(
				...
				TerminalContextKeys.terminalDictationInProgress.negate()
			),
			isHiddenByDefault: true
		});
		MenuRegistry.appendMenuItem(menuId, {
...
export function getTerminalActionBarArgs(
	...
	const splitLocation =
		...
		(typeof location === 'object' && hasKey(location, { viewColumn: true }) && location.viewColumn === ACTIVE_GROUP)
	...
	dropdownActions.push(
		disposableStore.add(
			new Action(TerminalCommandId.New, terminalStrings.new, undefined, true, () =>
	...
	dropdownActions.push(
		disposableStore.add(
			new Action(TerminalCommandId.NewInNewWindow, terminalStrings.newInNewWindow.value, undefined, true, () =>
				terminalService.createAndFocusTerminal({
					location: {
	...
	dropdownActions.push(
		disposableStore.add(
			new Action(TerminalCommandId.Split, terminalStrings.split.value, undefined, true, () =>
				terminalService.createAndFocusTerminal({
	...
	if (
		(aiProfiles.length > 0 || aiContributedProfiles.length > 0) &&
		(otherProfiles.length > 0 || otherContributedProfiles.length > 0)
	) {
		dropdownActions.push(new Separator());
	}
	...
	if (dropdownActions.length > 0) {
		dropdownActions.push(
			new SubmenuAction('split.profile', localize('split.profile', 'Split Terminal with Profile'), submenuActions)
		);
		dropdownActions.push(new Separator());
	}
	...
	const dropdownAction = disposableStore.add(
		new Action('refresh profiles', localize('launchProfile', 'Launch Profile...'), 'codicon-chevron-down', true)
	);
	return {
		dropdownAction,
...
function splitContributedProfiles(
	contributedProfiles: readonly IExtensionTerminalProfile[]
): [IExtensionTerminalProfile[], IExtensionTerminalProfile[]] {
