...
const checkListIcon = registerIcon(
	...
	Codicon.check,
	localize('checkListIcon', 'Icon for the checkmark in the workspace trust editor.')
);
...
const folderPickerIcon = registerIcon(
	...
	Codicon.folder,
	localize('folderPickerIcon', 'Icon for the pick folder icon in the workspace trust editor.')
);
const editIcon = registerIcon(
	'workspace-trust-editor-edit-folder',
	Codicon.edit,
	localize('editIcon', 'Icon for the edit folder icon in the workspace trust editor.')
);
const removeIcon = registerIcon(
	'workspace-trust-editor-remove-folder',
	Codicon.close,
	localize('removeIcon', 'Icon for the remove folder icon in the workspace trust editor.')
);
...
class WorkspaceTrustedUrisTable extends Disposable {
	...
	constructor(
		...
		const addButton = this._register(
			buttonBar.addButton({ title: localize('addButton', 'Add Folder'), ...defaultButtonStyles })
		);
	...
	layout(): void {
		this.table.layout(
			this.trustedUriEntries.length * TrustedUriTableVirtualDelegate.ROW_HEIGHT +
...
class TrustedUriPathColumnRenderer implements ITableRenderer<ITrustedUriItem, ITrustedUriPathColumnTemplateData> {
	...
	renderElement(item: ITrustedUriItem, index: number, templateData: ITrustedUriPathColumnTemplateData): void {
		...
		templateData.renderDisposables.add(
			addDisposableListener(templateData.pathInput.element, EventType.DBLCLICK, e => {
				EventHelper.stop(e);
			})
		);
		...
		templateData.renderDisposables.add(
			addStandardDisposableListener(templateData.pathInput.inputElement, EventType.KEY_DOWN, e => {
				let handled = false;
		...
		templateData.renderDisposables.add(
			addDisposableListener(templateData.pathInput.inputElement, EventType.BLUR, () => {
				reject();
			})
		);
...
export class WorkspaceTrustEditor extends EditorPane {
	...
	private async render() {
		...
		const headerDescriptionActionsText = localize(
			{
				...
				comment: ['Please ensure the markdown link syntax is not broken up with whitespace [text block](link block)']
			},
		...
		for (const node of parseLinkedText(headerDescriptionActionsText).nodes) {
			if (typeof node === 'string') {
				...
				this.rerenderDisposables.add(
					this.instantiationService.createInstance(Link, headerDescriptionActions, { ...node, tabIndex: -1 }, {})
				);
			}
		}
		...
		const settingsRequiringTrustedWorkspaceCount = restrictedSettings.default.filter(key => {
			...
			if (
				property.scope &&
				(APPLICATION_SCOPES.includes(property.scope) || property.scope === ConfigurationScope.MACHINE)
			) {
				return false;
			}
	...
	private getExtensionCount(): number {
		...
		for (const extension of localExtensions) {
			...
			if (
				this.extensionManifestPropertiesService.getExtensionUntrustedWorkspaceSupportType(extension.manifest) !== true
			) {
			...
			if (
				dependencies.some(
					ext =>
						this.extensionManifestPropertiesService.getExtensionUntrustedWorkspaceSupportType(ext.manifest) === false
				)
			) {
				set.add(extension.identifier.id);
			}
	...
	private createConfigurationElement(parent: HTMLElement): void {
		...
		this.workspaceTrustedUrisTable = this._register(
			this.instantiationService.createInstance(WorkspaceTrustedUrisTable, this.configurationContainer)
		);
	}
	...
	private createButtonRow(
		...
		for (const { action, keybinding } of buttonInfo) {
			...
			button.element.ariaLabel =
				action.label + ', ' + localize('keyboardShortcut', 'Keyboard Shortcut: {0}', keybinding.getAriaLabel()!);
	...
	private addTrustButtonToElement(parent: HTMLElement): void {
		const trustAction = this.rerenderDisposables.add(
		...
		if (this.workspaceTrustManagementService.canSetParentFolderTrust()) {
			...
			const trustParentAction = this.rerenderDisposables.add(
				new Action(
	...
	private addDontTrustButtonToElement(parent: HTMLElement): void {
		this.createButtonRow(parent, [
			{
				action: this.rerenderDisposables.add(
					new Action(
		...
		for (const limitation of limitations) {
			...
			for (const node of linkedText.nodes) {
				if (typeof node === 'string') {
					...
					this.rerenderDisposables.add(
						this.instantiationService.createInstance(Link, text, { ...node, tabIndex: -1 }, {})
					);
				}
			}
		}
