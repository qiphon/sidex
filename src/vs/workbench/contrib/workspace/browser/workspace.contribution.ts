...
export class WorkspaceTrustContextKeys extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(
			workspaceTrustManagementService.onDidChangeTrust(trusted => this._ctxWorkspaceTrustState.set(trusted))
		);
	}
}
...
export class WorkspaceTrustRequestHandler extends Disposable implements IWorkbenchContribution {
	...
	private registerListeners(): void {
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateOpenFilesTrustRequest(async () => {
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateResourcesTrustRequest(async options => {
				await this.workspaceTrustManagementService.workspaceResolved;
				...
				await this.dialogService.prompt<void>({
					...
					buttons: [
						{
							label: localize(
								{ key: 'trustResources', comment: ['&& denotes a mnemonic'] },
								'&&Trust Folder & Continue'
							),
							run: () =>
								this.workspaceTrustRequestService.completeResourcesTrustRequest(
									options.uri,
									WorkspaceTrustUriResponse.Open
								)
						}
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequest(async requestOptions => {
				await this.workspaceTrustManagementService.workspaceResolved;
...
export class WorkspaceTrustUXHandler extends Disposable implements IWorkbenchContribution {
	...
	private registerListeners(): void {
				...
				const addWorkspaceFolder = async (e: IWorkspaceFoldersWillChangeEvent): Promise<void> => {
					...
					if (trusted && (e.changes.added.length || e.changes.changed.length)) {
						const addedFoldersTrustInfo = await Promise.all(
							e.changes.added.map(folder => this.workspaceTrustManagementService.getUriTrustInfo(folder.uri))
						);
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequestOnStartup(async () => {
				...
				this.doShowModal(
					...
					},
					{
						...
						sublabel: isSingleFolderWorkspace
							? localize('dontTrustFolderOptionDescription', 'Open folder in restricted mode')
							: localize('dontTrustWorkspaceOptionDescription', 'Open workspace in restricted mode')
					},
					[
	...
	private updateStatusbarEntry(trusted: boolean): void {
		...
		if (!trusted && !this.statusbarEntryAccessor.value) {
			...
			this.statusbarEntryAccessor.value = this.statusbarService.addEntry(entry, this.entryId, StatusbarAlignment.LEFT, {
		...
		constructor() {
			super({
				...
				precondition: ContextKeyExpr.and(
					...
					ContextKeyExpr.equals(`config.${WORKSPACE_TRUST_ENABLED}`, true)
				),
...
class WorkspaceTrustTelemetryContribution extends Disposable implements IWorkbenchContribution {
	constructor(
		...
		this.workspaceTrustManagementService.workspaceTrustInitialized.then(() => {
			...
			this._register(
				this.workspaceTrustManagementService.onDidChangeTrust(isTrusted => this.logWorkspaceTrust(isTrusted))
			);
		});
	}
	...
	private logInitialWorkspaceTrustInfo(): void {
		if (!this.workspaceTrustEnablementService.isWorkspaceTrustEnabled()) {
			...
			this.telemetryService.publicLog2<WorkspaceTrustDisabledEvent, WorkspaceTrustDisabledEventClassification>(
		...
		this.telemetryService.publicLog2<WorkspaceTrustInfoEvent, WorkspaceTrustInfoEventClassification>(
	...
	private async logWorkspaceTrust(isTrusted: boolean): Promise<void> {
		...
		this.telemetryService.publicLog2<WorkspaceTrustStateChangedEvent, WorkspaceTrustStateChangedEventClassification>(
		...
		if (isTrusted) {
			...
			for (const folder of this.workspaceContextService.getWorkspace().folders) {
				...
				this.telemetryService.publicLog2<WorkspaceTrustFolderInfoEvent, WorkspaceTrustFolderInfoEventClassification>(
					'workspaceFolderDepthBelowTrustedFolder',
					{ workspaceFolderDepth, trustedFolderDepth, delta }
				);
			}
		}
	}
