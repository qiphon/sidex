...
export class SetLogLevelAction extends Action {
			...
			disposables.add(
				quickPick.onDidHide(() => {
					const result = selectedItem
						? ((<LogLevelQuickPickItem>selectedItem).level ?? <LogChannelQuickPickItem>selectedItem)
						: null;
					disposables.dispose();
					resolve(result);
				})
	...
	private async setLogLevelForChannel(logChannel: LogChannelQuickPickItem): Promise<void> {
		...
		const defaultLogLevel =
			defaultLogLevels.extensions.find(e => e[0] === logChannel.channel.extensionId?.toLowerCase())?.[1] ??
	...
	override async run(): Promise<void> {
		...
		if (sessionResult) {
			const logFileResult = await this.quickInputService.pick(
				this.getLogFiles(URI.parse(sessionResult.id!)).then(logFiles =>
					logFiles.map(
						(s): IQuickPickItem => ({
							id: s.toString(),
							label: basename(s)
