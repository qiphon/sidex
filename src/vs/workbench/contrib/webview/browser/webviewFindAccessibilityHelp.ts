...
class WebviewFindAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	provideContent(): string {
		...
		content.push(
			localize('webview.hear1', '1) The content containing the match is read first, so you get immediate context.')
		);
		content.push(
			localize(
				'webview.hear2',
		...
		content.push(
			localize('webview.hear3', '3) The exact location information is announced so you know where the match is.')
		);
		content.push('');
		...
		content.push(
			localize('webview.focusDesc2', 'The webview may scroll to show the match, depending on how it is designed.')
		);
		content.push(
			localize(
				'webview.focusDesc3',
		...
		content.push(
			localize('webview.keyShiftEnter', '- Shift+Enter: Move to the previous match while staying in the Find input.')
		);
		content.push('');
