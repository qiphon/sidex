...
export const accessibleViewHasUnassignedKeybindings = new RawContextKey<boolean>(
...
function getDelaysFromConfig(
	...
	type: 'general' | 'errorAtPosition' | 'warningAtPosition'
): { announcement: number; sound: number } | undefined {
	return (
...
Registry.as<IConfigurationMigrationRegistry>(
...
).registerConfigurationMigrations([
	{
		...
		migrateFn: (value: boolean) => {
			...
			return [[AccessibilityVoiceSettingId.AutoSynthesize, { value: newValue }]];
		}
	}
]);
...
Registry.as<IConfigurationMigrationRegistry>(
	...
	AccessibilitySignal.allAccessibilitySignals
		.map<ConfigurationMigration | undefined>(item =>
