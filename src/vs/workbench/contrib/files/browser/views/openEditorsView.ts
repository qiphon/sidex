...
export class OpenEditorsView extends ViewPane {
			...
			],
			{
				identityProvider: {
					getId: (element: OpenEditor | IEditorGroup) =>
						element instanceof OpenEditor ? element.getId() : element.id.toString()
				},
				dnd: this.dnd,
		...
		this.editorGroupService.getGroups(GroupsOrder.GRID_APPEARANCE).forEach(g => {
			...
			if (this.sortOrder === 'alphabetical') {
				editors = editors.sort((first, second) =>
					compareFileNamesDefault(first.editor.getName(), second.editor.getName())
				);
		...
		this.contextMenuService.showContextMenu({
			...
			getActionsContext: () =>
				element instanceof OpenEditor
					? { groupId: element.groupId, editorIndex: element.group.getIndexOfEditor(element.editor) }
					: { groupId: element.id }
		});
	}
	...
	private updateSize(): void {
		...
		this.maximumBodySize =
			this.orientation === Orientation.VERTICAL ? this.getMaxExpandedBodySize() : Number.POSITIVE_INFINITY;
	}
...
class OpenEditorRenderer implements IListRenderer<OpenEditor, IOpenEditorTemplateData> {
	...
	constructor(
		...
		this.closeEditorAction = this.instantiationService.createInstance(
			CloseEditorAction,
			CloseEditorAction.ID,
			CloseEditorAction.LABEL
		);
		this.unpinEditorAction = this.instantiationService.createInstance(
			UnpinEditorAction,
...
class OpenEditorsDragAndDrop implements IListDragAndDrop<OpenEditor | IEditorGroup> {
	...
	drop(
		...
		let targetEditorIndex =
			targetElement instanceof OpenEditor ? targetElement.group.getIndexOfEditor(targetElement.editor) : 0;
