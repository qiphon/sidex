...
abstract class BaseEditorOption<K extends EditorOption, T, V> implements IEditorOption<K, V> {
	...
	constructor(
		...
		defaultValue: V,
		schema?: IConfigurationPropertySchema | { [path: string]: IConfigurationPropertySchema }
	) {
		this.id = id;
		this.name = name;
	...
	public validate(_input: unknown): EditorFindOptions {
		...
		return {
			...
			seedSearchStringFromSelection:
				typeof input.seedSearchStringFromSelection === 'boolean'
			...
			autoFindInSelection:
				typeof input.autoFindInSelection === 'boolean'
			...
			replaceHistory: stringSet<'never' | 'workspace'>(input.replaceHistory, this.defaultValue.replaceHistory, [
				'never',
				'workspace'
			])
		};
...
> {
	...
	public validate(_input: unknown): GoToLocationOptions {
		...
		return {
			multiple: stringSet<GoToLocationValues>(input.multiple, this.defaultValue.multiple, [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
			multipleDefinitions: stringSet<GoToLocationValues>(input.multipleDefinitions, 'peek', [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
			multipleTypeDefinitions: stringSet<GoToLocationValues>(input.multipleTypeDefinitions, 'peek', [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
			multipleDeclarations: stringSet<GoToLocationValues>(input.multipleDeclarations, 'peek', [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
			multipleImplementations: stringSet<GoToLocationValues>(input.multipleImplementations, 'peek', [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
			multipleReferences: stringSet<GoToLocationValues>(input.multipleReferences, 'peek', [
				'peek',
				'gotoAndPeek',
				'goto'
			]),
...
class EditorHover extends BaseEditorOption<EditorOption.hover, IEditorHoverOptions, EditorHoverOptions> {
			...
			'editor.hover.enabled': {
				...
				markdownEnumDescriptions: [
					...
					nls.localize(
						...
						platform.isMacintosh ? `Command` : `Control`
					)
				],
	...
	public validate(_input: unknown): EditorHoverOptions {
		...
		return {
			enabled: stringSet<'on' | 'off' | 'onKeyboardModifier'>(input.enabled, this.defaultValue.enabled, [
				'on',
				'off',
				'onKeyboardModifier'
			]),
...
export class EditorLayoutInfoComputer extends ComputedEditorOption<EditorOption.layoutInfo, EditorLayoutInfo> {
	...
	public static computeContainedMinimapLineCount(input: {
		...
		const desiredRatio =
			(extraLinesBeforeFirstLine + input.viewLineCount + extraLinesBeyondLastLine) / (input.pixelRatio * input.height);
			...
			if (ratio > 1) {
				...
				if (minimapSize === 'fill' || fitBecomesFill) {
					...
					minimapCanvasInnerHeight = Math.ceil(
						Math.max(typicalViewportLineCount, extraLinesBeforeFirstLine + viewLineCount + extraLinesBeyondLastLine) *
							minimapLineHeight
					);
	...
	public static computeLayout(options: IComputedEditorOptions, env: EditorLayoutInfoComputerEnv): EditorLayoutInfo {
		...
		if (
			options.get(EditorOption.accessibilitySupport) === AccessibilitySupport.Enabled &&
...
> {
	...
	public validate(_input: unknown): EditorStickyScrollOptions {
		...
		return {
			...
			defaultModel: stringSet<'outlineModel' | 'foldingProviderModel' | 'indentationModel'>(
				input.defaultModel,
...
class EditorMinimap extends BaseEditorOption<EditorOption.minimap, IEditorMinimapOptions, EditorMinimapOptions> {
	...
	public validate(_input: unknown): EditorMinimapOptions {
		...
		return {
			enabled: boolean(input.enabled, this.defaultValue.enabled),
			autohide: stringSet<'none' | 'mouseover' | 'scroll'>(input.autohide, this.defaultValue.autohide, [
				'none',
				'mouseover',
				'scroll'
			]),
			size: stringSet<'proportional' | 'fill' | 'fit'>(input.size, this.defaultValue.size, [
				'proportional',
				'fill',
				'fit'
			]),
			side: stringSet<'right' | 'left'>(input.side, this.defaultValue.side, ['right', 'left']),
			showSlider: stringSet<'always' | 'mouseover'>(input.showSlider, this.defaultValue.showSlider, [
				'always',
				'mouseover'
			]),
			...
			sectionHeaderLetterSpacing: EditorFloatOption.clamp(
				EditorFloatOption.float(input.sectionHeaderLetterSpacing, this.defaultValue.sectionHeaderLetterSpacing),
				0,
				5
			)
		};
...
> {
		...
		const types: IJSONSchema[] = [
			{ type: 'boolean' },
			{
				...
				enumDescriptions: [
					...
					nls.localize('offWhenInlineCompletions', 'Quick suggestions are disabled when inline completions are showing')
				]
			}
		];
		super(EditorOption.quickSuggestions, 'quickSuggestions', defaults, {
			anyOf: [
				{ type: 'boolean' },
				{
					...
					enum: ['on', 'inline', 'off', 'offWhenInlineCompletions'],
					enumDescriptions: [
						nls.localize('quickSuggestions.topLevel.on', 'Quick suggestions are enabled for all token types'),
						nls.localize(
							'quickSuggestions.topLevel.inline',
							'Quick suggestions show as ghost text for all token types'
						),
						nls.localize('quickSuggestions.topLevel.off', 'Quick suggestions are disabled for all token types'),
						nls.localize(
	...
	public validate(input: unknown): InternalQuickSuggestionsOptions {
		...
		if (typeof input === 'string') {
			...
			const validated = stringSet<QuickSuggestionsValue>(
				input as QuickSuggestionsValue,
...
> {
	...
	public validate(_input: unknown): InternalUnicodeHighlightOptions {
		...
		return {
			nonBasicASCII: primitiveSet<boolean | InUntrustedWorkspace>(input.nonBasicASCII, inUntrustedWorkspace, [
				true,
				false,
			...
			includeComments: primitiveSet<boolean | InUntrustedWorkspace>(input.includeComments, inUntrustedWorkspace, [
				true,
				false,
				inUntrustedWorkspace
			]),
			includeStrings: primitiveSet<boolean | InUntrustedWorkspace>(input.includeStrings, inUntrustedWorkspace, [
				true,
				false,
				inUntrustedWorkspace
			]),
...
> {
	...
	private _validateEdits(_input: unknown): InternalInlineSuggestOptions['edits'] {
		...
		return {
			...
			allowCodeShifting: stringSet(input.allowCodeShifting, this.defaultValue.edits.allowCodeShifting, [
				'always',
				'horizontal',
				'never'
			]),
	...
	constructor() {
		const defaults: InternalBracketPairColorizationOptions = {
			...
			independentColorPoolPerBracketType:
				EDITOR_MODEL_DEFAULTS.bracketPairColorizationOptions.independentColorPoolPerBracketType
		};
...
class GuideOptions extends BaseEditorOption<EditorOption.guides, IGuidesOptions, InternalGuidesOptions> {
	...
	public validate(_input: unknown): InternalGuidesOptions {
		...
		return {
			...
			bracketPairsHorizontal: primitiveSet(input.bracketPairsHorizontal, this.defaultValue.bracketPairsHorizontal, [
				true,
				false,
				'active'
			]),
...
export const EditorOptions = {
	acceptSuggestionOnCommitCharacter: register(
		new EditorBooleanOption(EditorOption.acceptSuggestionOnCommitCharacter, 'acceptSuggestionOnCommitCharacter', true, {
			markdownDescription: nls.localize(
				'acceptSuggestionOnCommitCharacter',
	...
	allowVariableLineHeights: register(
		new EditorBooleanOption(EditorOption.allowVariableLineHeights, 'allowVariableLineHeights', true, {
			description: nls.localize(
				'allowVariableLineHeights',
	...
	allowVariableFonts: register(
		new EditorBooleanOption(EditorOption.allowVariableFonts, 'allowVariableFonts', true, {
			description: nls.localize('allowVariableFonts', 'Controls whether to allow using variable fonts in the editor.')
		})
	),
	allowVariableFontsInAccessibilityMode: register(
	...
	autoIndentOnPaste: register(
		new EditorBooleanOption(EditorOption.autoIndentOnPaste, 'autoIndentOnPaste', false, {
			description: nls.localize(
				'autoIndentOnPaste',
	...
	autoIndentOnPasteWithinString: register(
		new EditorBooleanOption(EditorOption.autoIndentOnPasteWithinString, 'autoIndentOnPasteWithinString', true, {
			description: nls.localize(
				'autoIndentOnPasteWithinString',
	...
	stickyTabStops: register(
		new EditorBooleanOption(EditorOption.stickyTabStops, 'stickyTabStops', false, {
			description: nls.localize(
				'stickyTabStops',
	...
	codeLensFontFamily: register(
		new EditorStringOption(EditorOption.codeLensFontFamily, 'codeLensFontFamily', '', {
			description: nls.localize('codeLensFontFamily', 'Controls the font family for CodeLens.')
		})
	),
	codeLensFontSize: register(
		new EditorIntOption(EditorOption.codeLensFontSize, 'codeLensFontSize', 0, 0, 100, {
			type: 'number',
			default: 0,
			minimum: 0,
			maximum: 100,
	...
	colorDecorators: register(
		new EditorBooleanOption(EditorOption.colorDecorators, 'colorDecorators', true, {
			description: nls.localize(
				'colorDecorators',
	...
	colorDecoratorsLimit: register(
		new EditorIntOption(EditorOption.colorDecoratorsLimit, 'colorDecoratorsLimit', 500, 1, 1000000, {
			markdownDescription: nls.localize(
				'colorDecoratorsLimit',
				'Controls the max number of color decorators that can be rendered in an editor at once.'
			)
	...
	columnSelection: register(
		new EditorBooleanOption(EditorOption.columnSelection, 'columnSelection', false, {
			description: nls.localize(
				'columnSelection',
				'Enable that the selection with the mouse and keys is doing column selection.'
			)
	...
	copyWithSyntaxHighlighting: register(
		new EditorBooleanOption(EditorOption.copyWithSyntaxHighlighting, 'copyWithSyntaxHighlighting', true, {
			description: nls.localize(
				'copyWithSyntaxHighlighting',
	...
	cursorWidth: register(
		new EditorIntOption(EditorOption.cursorWidth, 'cursorWidth', 0, 0, Constants.MAX_SAFE_SMALL_INTEGER, {
			markdownDescription: nls.localize(
				'cursorWidth',
	...
	cursorHeight: register(
		new EditorIntOption(EditorOption.cursorHeight, 'cursorHeight', 0, 0, Constants.MAX_SAFE_SMALL_INTEGER, {
			markdownDescription: nls.localize(
				'cursorHeight',
	...
	disableLayerHinting: register(
		new EditorBooleanOption(EditorOption.disableLayerHinting, 'disableLayerHinting', false)
	),
	disableMonospaceOptimizations: register(
		new EditorBooleanOption(EditorOption.disableMonospaceOptimizations, 'disableMonospaceOptimizations', false)
	),
	domReadOnly: register(new EditorBooleanOption(EditorOption.domReadOnly, 'domReadOnly', false)),
	doubleClickSelectsBlock: register(
		new EditorBooleanOption(EditorOption.doubleClickSelectsBlock, 'doubleClickSelectsBlock', true, {
			description: nls.localize(
				'doubleClickSelectsBlock',
				'Controls whether double-clicking next to a bracket or quote selects the content inside.'
			)
	...
	renderRichScreenReaderContent: register(
		new EditorBooleanOption(EditorOption.renderRichScreenReaderContent, 'renderRichScreenReaderContent', false, {
			markdownDescription: nls.localize(
				'renderRichScreenReaderContent',
	...
	fastScrollSensitivity: register(
		new EditorFloatOption(EditorOption.fastScrollSensitivity, 'fastScrollSensitivity', 5, x => (x <= 0 ? 5 : x), {
	...
	fixedOverflowWidgets: register(
		new EditorBooleanOption(EditorOption.fixedOverflowWidgets, 'fixedOverflowWidgets', false)
	),
	folding: register(
	...
	foldingHighlight: register(
		new EditorBooleanOption(EditorOption.foldingHighlight, 'foldingHighlight', true, {
	...
	foldingImportsByDefault: register(
		new EditorBooleanOption(EditorOption.foldingImportsByDefault, 'foldingImportsByDefault', false, {
			description: nls.localize(
				'foldingImportsByDefault',
	...
	unfoldOnClickAfterEndOfLine: register(
		new EditorBooleanOption(EditorOption.unfoldOnClickAfterEndOfLine, 'unfoldOnClickAfterEndOfLine', false, {
			description: nls.localize(
				'unfoldOnClickAfterEndOfLine',
				'Controls whether clicking on the empty content after a folded line will unfold the line.'
			)
	...
	fontFamily: register(
		new EditorStringOption(EditorOption.fontFamily, 'fontFamily', EDITOR_FONT_DEFAULTS.fontFamily, {
	...
	formatOnPaste: register(
		new EditorBooleanOption(EditorOption.formatOnPaste, 'formatOnPaste', false, {
			description: nls.localize(
				'formatOnPaste',
	...
	hideCursorInOverviewRuler: register(
		new EditorBooleanOption(EditorOption.hideCursorInOverviewRuler, 'hideCursorInOverviewRuler', false, {
			description: nls.localize(
				'hideCursorInOverviewRuler',
	...
	inertialScroll: register(
		new EditorBooleanOption(EditorOption.inertialScroll, 'inertialScroll', false, {
	...
	lineNumbersMinChars: register(
		new EditorIntOption(EditorOption.lineNumbersMinChars, 'lineNumbersMinChars', 5, 1, 300)
	),
	linkedEditing: register(
		new EditorBooleanOption(EditorOption.linkedEditing, 'linkedEditing', false, {
			description: nls.localize(
				'linkedEditing',
	...
	mouseWheelZoom: register(
		new EditorBooleanOption(EditorOption.mouseWheelZoom, 'mouseWheelZoom', false, {
	...
	multiCursorMergeOverlapping: register(
		new EditorBooleanOption(EditorOption.multiCursorMergeOverlapping, 'multiCursorMergeOverlapping', true, {
	...
	mouseMiddleClickAction: register(
		new EditorStringEnumOption(
			EditorOption.mouseMiddleClickAction,
			'mouseMiddleClickAction',
			'default' as MouseMiddleClickAction,
	...
	multiCursorLimit: register(
		new EditorIntOption(EditorOption.multiCursorLimit, 'multiCursorLimit', 10000, 1, 100000, {
			markdownDescription: nls.localize(
				'multiCursorLimit',
	...
	occurrencesHighlightDelay: register(
		new EditorIntOption(EditorOption.occurrencesHighlightDelay, 'occurrencesHighlightDelay', 0, 0, 2000, {
			description: nls.localize(
				'occurrencesHighlightDelay',
	...
	overtypeOnPaste: register(
		new EditorBooleanOption(EditorOption.overtypeOnPaste, 'overtypeOnPaste', true, {
			description: nls.localize('overtypeOnPaste', 'Controls whether pasting should overtype.')
		})
	),
	overviewRulerBorder: register(
		new EditorBooleanOption(EditorOption.overviewRulerBorder, 'overviewRulerBorder', true, {
			description: nls.localize(
				'overviewRulerBorder',
	...
	definitionLinkOpensInPeek: register(
		new EditorBooleanOption(EditorOption.definitionLinkOpensInPeek, 'definitionLinkOpensInPeek', false, {
			description: nls.localize(
				'definitionLinkOpensInPeek',
	...
	renderControlCharacters: register(
		new EditorBooleanOption(EditorOption.renderControlCharacters, 'renderControlCharacters', true, {
			description: nls.localize(
				'renderControlCharacters',
	...
	renderLineHighlightOnlyWhenFocus: register(
		new EditorBooleanOption(EditorOption.renderLineHighlightOnlyWhenFocus, 'renderLineHighlightOnlyWhenFocus', false, {
			description: nls.localize(
				'renderLineHighlightOnlyWhenFocus',
	...
	revealHorizontalRightPadding: register(
		new EditorIntOption(EditorOption.revealHorizontalRightPadding, 'revealHorizontalRightPadding', 15, 0, 1000)
	),
	roundedSelection: register(
		new EditorBooleanOption(EditorOption.roundedSelection, 'roundedSelection', true, {
	...
	scrollBeyondLastLine: register(
		new EditorBooleanOption(EditorOption.scrollBeyondLastLine, 'scrollBeyondLastLine', true, {
			description: nls.localize('scrollBeyondLastLine', 'Controls whether the editor will scroll beyond the last line.')
		})
	),
	scrollOnMiddleClick: register(
		new EditorBooleanOption(EditorOption.scrollOnMiddleClick, 'scrollOnMiddleClick', false, {
			description: nls.localize(
				'scrollOnMiddleClick',
	...
	scrollPredominantAxis: register(
		new EditorBooleanOption(EditorOption.scrollPredominantAxis, 'scrollPredominantAxis', true, {
			description: nls.localize(
				'scrollPredominantAxis',
	...
	selectionClipboard: register(
		new EditorBooleanOption(EditorOption.selectionClipboard, 'selectionClipboard', true, {
			description: nls.localize(
				'selectionClipboard',
	...
	selectionHighlight: register(
		new EditorBooleanOption(EditorOption.selectionHighlight, 'selectionHighlight', true, {
			description: nls.localize(
				'selectionHighlight',
	...
	selectionHighlightMaxLength: register(
		new EditorIntOption(
	...
	selectionHighlightMultiline: register(
		new EditorBooleanOption(EditorOption.selectionHighlightMultiline, 'selectionHighlightMultiline', false, {
			description: nls.localize(
				'selectionHighlightMultiline',
	...
	showDeprecated: register(
		new EditorBooleanOption(EditorOption.showDeprecated, 'showDeprecated', true, {
	...
	smoothScrolling: register(
		new EditorBooleanOption(EditorOption.smoothScrolling, 'smoothScrolling', false, {
	...
	suggestFontSize: register(
		new EditorIntOption(EditorOption.suggestFontSize, 'suggestFontSize', 0, 0, 1000, {
			markdownDescription: nls.localize(
				'suggestFontSize',
	...
	suggestLineHeight: register(
		new EditorIntOption(EditorOption.suggestLineHeight, 'suggestLineHeight', 0, 0, 1000, {
			markdownDescription: nls.localize(
				'suggestLineHeight',
	...
	suggestOnTriggerCharacters: register(
		new EditorBooleanOption(EditorOption.suggestOnTriggerCharacters, 'suggestOnTriggerCharacters', true, {
			description: nls.localize(
				'suggestOnTriggerCharacters',
	...
	trimWhitespaceOnDelete: register(
		new EditorBooleanOption(EditorOption.trimWhitespaceOnDelete, 'trimWhitespaceOnDelete', false, {
			description: nls.localize(
				'trimWhitespaceOnDelete',
	...
	wordSeparators: register(
		new EditorStringOption(EditorOption.wordSeparators, 'wordSeparators', USUAL_WORD_SEPARATORS, {
			description: nls.localize(
				'wordSeparators',
	...
	wordWrapColumn: register(
		new EditorIntOption(EditorOption.wordWrapColumn, 'wordWrapColumn', 80, 1, Constants.MAX_SAFE_SMALL_INTEGER, {
			markdownDescription: nls.localize(
				{
					key: 'wordWrapColumn',
					comment: [
	...
	wrapOnEscapedLineFeeds: register(
		new EditorBooleanOption(EditorOption.wrapOnEscapedLineFeeds, 'wrapOnEscapedLineFeeds', false, {
			markdownDescription: nls.localize(
				'wrapOnEscapedLineFeeds',
