...
export class ViewportRenderStrategy extends BaseRenderStrategy {
	...
	update(viewportData: ViewportData, viewLineOptions: ViewLineOptions): number {
		...
		for (y = viewportData.startLineNumber; y <= viewportData.endLineNumber; y++) {
			...
			fillStartIndex =
				((y - viewportData.startLineNumber) * ViewportRenderStrategy.maxSupportedColumns + tokenEndIndex) *
				Constants.IndicesPerCell;
			fillEndIndex =
				(y - viewportData.startLineNumber) * ViewportRenderStrategy.maxSupportedColumns * Constants.IndicesPerCell;
