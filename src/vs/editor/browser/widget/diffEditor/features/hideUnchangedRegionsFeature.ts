...
export class HideUnchangedRegionsFeature extends Disposable {
	...
	public static setBreadcrumbsSourceFactory(
		factory: (textModel: ITextModel, instantiationService: IInstantiationService) => IDiffEditorBreadcrumbsSource
	) {
				...
				if (compactMode) {
					{
						const d = derived(
							this,
							reader =>
								/** @description hiddenOriginalRangeStart */ r.getHiddenOriginalRange(reader).startLineNumber - 1
						);
					...
					}
					{
						const d = derived(
							this,
							reader =>
								/** @description hiddenModifiedRangeStart */ r.getHiddenModifiedRange(reader).startLineNumber - 1
						);
			...
			autorun(reader => {
				...
				try {
					this._editors.original.setHiddenAreas(
						curUnchangedRegions.map(r => r.getHiddenOriginalRange(reader).toInclusiveRange()).filter(isDefined)
					);
					this._editors.modified.setHiddenAreas(
						curUnchangedRegions.map(r => r.getHiddenModifiedRange(reader).toInclusiveRange()).filter(isDefined)
					);
				} finally {
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
	...
	constructor(
		...
		if (!this._hide) {
			this._register(
				applyStyle(this._nodes.first, { width: observableCodeEditor(this._editor).layoutInfoContentLeft })
			);
		} else {
		...
		this._register(
			addDisposableListener(this._nodes.bottom, 'mousedown', e => {
				...
				const mouseMoveListener = addDisposableListener(window, 'mousemove', e => {
					...
					const top =
						this._unchangedRegionRange.endLineNumberExclusive > editor.getModel()!.getLineCount()
		...
		this._register(
			autorun(reader => {
				...
				if (!this._hide) {
					...
					const span = $(
						'span',
						{ title: localize('diff.hiddenLines.expandAll', 'Double click to unfold') },
						linesHiddenText
					);
