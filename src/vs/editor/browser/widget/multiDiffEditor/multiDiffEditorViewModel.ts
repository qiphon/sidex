...
export class MultiDiffEditorViewModel extends Disposable {
	...
	constructor(
		...
		this.activeDiffItem = derivedObservableWithWritableCache<DocumentDiffItemViewModel | undefined>(
			this,
			(reader, lastValue) =>
				(this.focusedDiffItem.read(reader) ?? (lastValue && this.items.read(reader).indexOf(lastValue) !== -1))
					? lastValue
					: undefined
		);
	}
...
export class DocumentDiffItemViewModel extends Disposable {
	...
	constructor(
		...
		const originalTextModel =
			this.documentDiffItem.original ?? diffEditorViewModelStore.add(this._modelService.createModel('', null));
		const modifiedTextModel =
			this.documentDiffItem.modified ?? diffEditorViewModelStore.add(this._modelService.createModel('', null));
