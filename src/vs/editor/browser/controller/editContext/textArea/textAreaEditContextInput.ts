...
export class TextAreaInput extends Disposable {
	...
	private readonly _asyncFocusGainWriteScreenReaderContent: MutableDisposable<RunOnceScheduler> = this._register(
	...
	constructor(
		...
		this._register(
			Event.runAndSubscribe(this._accessibilityService.onDidChangeScreenReaderOptimized, () => {
				if (
...
export class TextAreaWrapper extends Disposable implements ICompleteTextAreaWrapper {
	...
	constructor(private readonly _actual: HTMLTextAreaElement) {
		...
		this._register(
			dom.addDisposableListener(this._actual, TextAreaSyntethicEvents.Tap, () => this._onSyntheticTap.fire())
		);
	}
