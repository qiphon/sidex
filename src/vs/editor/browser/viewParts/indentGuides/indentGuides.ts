...
export class IndentGuidesOverlay extends DynamicViewOverlay {
	...
	public override onLanguageConfigurationChanged(_e: viewEvents.ViewLanguageConfigurationEvent): boolean {
		return true;
	}
		...
		for (let lineNumber = visibleStartLineNumber; lineNumber <= visibleEndLineNumber; lineNumber++) {
			...
			for (const guide of indent) {
				...
				const width = guide.horizontalLine
					? (ctx.visibleRangeForPosition(new Position(lineNumber, guide.horizontalLine.endColumn))?.left ??
							left + this._spaceWidth) - left
					: this._spaceWidth;
	...
	private getGuidesByLine(
			...
			this._bracketPairGuideOptions.bracketPairs !== false
				? this._context.viewModel.getBracketGuidesInRangeByLine(
						...
						activeCursorPosition,
						{
							...
							horizontalGuides:
								this._bracketPairGuideOptions.bracketPairsHorizontal === true
		...
		for (let lineNumber = visibleStartLineNumber; lineNumber <= visibleEndLineNumber; lineNumber++) {
			...
			lineGuides.push(...(bracketGuidesInLineQueue.takeWhile(_g => true) || []));
		}
