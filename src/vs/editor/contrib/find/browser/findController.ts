		...
		if (this._model) {
			if (this._editor.getModel()?.isTooLargeForHeapOperation()) {
				this._notificationService.warn(
					nls.localize('too.large.for.replaceall', 'The file is too large to perform a replace all operation.')
				);
				return false;
			}
	...
	(accessor: ServicesAccessor, editor: ICodeEditor, _args: any): boolean | Promise<void> => {
		...
		return controller.start({
			...
			seedSearchStringFromSelection:
				editor.getOption(EditorOption.find).seedSearchStringFromSelection !== 'never' ? 'single' : 'none',
			seedSearchStringFromNonEmptySelection:
				editor.getOption(EditorOption.find).seedSearchStringFromSelection === 'selection',
...
export const NextMatchFindAction = registerMultiEditorAction(
	new MultiEditorAction({
...
export const PreviousMatchFindAction = registerMultiEditorAction(
	new MultiEditorAction({
