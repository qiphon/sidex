...
const findCollapsedIcon = registerIcon(
	...
	nls.localize('findCollapsedIcon', 'Icon to indicate that the editor find widget is collapsed.')
);
const findExpandedIcon = registerIcon(
	'find-expanded',
	Codicon.chevronDown,
	nls.localize('findExpandedIcon', 'Icon to indicate that the editor find widget is expanded.')
);

export const findSelectionIcon = registerIcon(
	'find-selection',
	Codicon.selection,
	nls.localize('findSelectionIcon', "Icon for 'Find in Selection' in the editor find widget.")
);
export const findReplaceIcon = registerIcon(
	'find-replace',
	Codicon.replace,
	nls.localize('findReplaceIcon', "Icon for 'Replace' in the editor find widget.")
);
export const findReplaceAllIcon = registerIcon(
	'find-replace-all',
	Codicon.replaceAll,
	nls.localize('findReplaceAllIcon', "Icon for 'Replace All' in the editor find widget.")
);
export const findPreviousMatchIcon = registerIcon(
	'find-previous-match',
	Codicon.arrowUp,
	nls.localize('findPreviousMatchIcon', "Icon for 'Find Previous' in the editor find widget.")
);
export const findNextMatchIcon = registerIcon(
	'find-next-match',
	Codicon.arrowDown,
	nls.localize('findNextMatchIcon', "Icon for 'Find Next' in the editor find widget.")
);
...
export class FindWidget extends Widget implements IOverlayWidget, IVerticalSashLayoutProvider {
	...
	constructor(
		...
		this._register(
			this._codeEditor.onDidChangeConfiguration((e: ConfigurationChangedEvent) => {
		...
		this._register(
			this._widgetFocusTracker.onDidFocus(() => {
				this._findWidgetFocused.set(true);
			})
		);
		this._register(
			this._widgetFocusTracker.onDidBlur(() => {
		...
		this._register(
			dom.addDisposableListener(this._domNode, 'focusin', (e: FocusEvent) => {
	...
	public focusLastElement(): void {
		...
		if (
			...
			this._domNode.contains(this._lastFocusedElement) &&
			dom.getWindow(this._lastFocusedElement).document.body.contains(this._lastFocusedElement)
		) {
			...
			new SimpleButton(
				{
					...
					onTrigger: () => {
						assertReturnsDefined(this._codeEditor.getAction(FIND_IDS.PreviousMatchFindAction))
							.run()
			...
			new SimpleButton(
				{
					...
					hoverLifecycleOptions,
					onTrigger: () => {
						assertReturnsDefined(this._codeEditor.getAction(FIND_IDS.NextMatchFindAction))
							.run()
							.then(undefined, onUnexpectedError);
					}
				},
...
export class SimpleButton extends Widget {
	...
	constructor(opts: ISimpleButtonOpts, hoverService: IHoverService) {
		...
		this._register(
			hoverService.setupDelayedHover(
				this._domNode,
				{
