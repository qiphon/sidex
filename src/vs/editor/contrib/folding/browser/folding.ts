		...
		if (this.updateScheduler) {
			...
			this.foldingModelPromise = this.updateScheduler
				.trigger(() => {
					...
					const foldingRegionPromise = (this.foldingRegionPromise = createCancelablePromise(token =>
			...
			if (iconClicked || isCollapsed) {
				...
				if (surrounding) {
					const filter = (otherRegion: FoldingRegion) =>
						!otherRegion.containedBy(region) && !region.containedBy(otherRegion);
...
function foldingArgumentsConstraint(args: unknown) {
	if (!types.isUndefined(args)) {
		...
		if (
			...
			(!Array.isArray(foldingArgs.selectionLines) || !foldingArgs.selectionLines.every(types.isNumber))
		) {
			return false;
		}
	}
