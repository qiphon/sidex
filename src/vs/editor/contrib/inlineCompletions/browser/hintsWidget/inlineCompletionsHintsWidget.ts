		...
		this._register(
			autorunWithStore((reader, store) => {
				...
				const contentWidgetValue = derived(reader => {
					const contentWidget = reader.store.add(
				...
				const hadPosition = derivedObservableWithCache(
					this,
					(reader, lastValue) => !!this.position.read(reader) || !!lastValue
				);
				store.add(
					autorun(reader => {
...
const inlineSuggestionHintsPreviousIcon = registerIcon(
	...
	Codicon.chevronLeft,
	localize('parameterHintsPreviousIcon', 'Icon for show previous parameter hint.')
);
...
export class InlineSuggestionHintsContentWidget extends Disposable implements IContentWidget {
	...
	constructor(
		...
		this.availableSuggestionCountAction = this._register(
			new Action('inlineSuggestionHints.availableSuggestionCount', '', undefined, false)
		);
		this.nextAction = this._register(
			this.createCommandAction(
