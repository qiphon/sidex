...
export class InlineEditsCustomView extends Disposable implements IInlineEditsView {
		...
		this._register(
			this._editorObs.createOverlayWidget({
				...
				minContentWidthInPx: derivedObservableWithCache<number>(this, (reader, prev) => {
					...
					const current =
						s.rect.map(rect => rect.right).read(reader) +
	...
	private fitsInsideViewport(range: LineRange, displayLabel: string, reader: IReader | undefined): boolean {
		...
		const minimapWidth =
			this._editorObs.layoutInfoMinimap.read(reader).minimapLeft !== 0
				? this._editorObs.layoutInfoMinimap.read(reader).minimapWidth
				: 0;
	...
	private getState(displayLocation: InlineSuggestHint): {
		...
		const contentState = derived(this, reader => {
			...
			const lineWidth = maxContentWidthInRange(
				this._editorObs,
				new LineRange(startLineNumber, startLineNumber + 1),
				reader
			);
			const lineWidthBelow =
				startLineNumber + 1 <= lineCount
					? maxContentWidthInRange(this._editorObs, new LineRange(startLineNumber + 1, startLineNumber + 2), reader)
					: undefined;
			const lineWidthAbove =
				startLineNumber - 1 >= 1
					? maxContentWidthInRange(this._editorObs, new LineRange(startLineNumber - 1, startLineNumber), reader)
					: undefined;
		...
		const rect = derived(this, reader => {
			...
			const { lineWidth, lineWidthBelow, lineWidthAbove, startContentLeftOffset, endContentLeftOffset } =
				contentState.read(reader);

			const contentLeft = this._editorObs.layoutInfoContentLeft.read(reader);
			const lineHeight = this._editorObs
				.observeLineHeightForLine(startLineNumber)
			...
			if (startLineNumber === endLineNumber && endContentLeftOffset + 5 * w >= lineWidth && fitsInsideViewport) {
				...
				lineWidthBelow !== undefined &&
				lineWidthBelow + MIN_END_OF_LINE_PADDING - HORIZONTAL_OFFSET_WHEN_ABOVE_BELOW - PADDING_HORIZONTALLY <
					startContentLeftOffset
			) {
				position = 'below'; // Render Below if possible
			} else if (
				lineWidthAbove !== undefined &&
				lineWidthAbove + MIN_END_OF_LINE_PADDING - HORIZONTAL_OFFSET_WHEN_ABOVE_BELOW - PADDING_HORIZONTALLY <
					startContentLeftOffset
			) {
	...
	private getRendering(
		state: { rect: IObservable<Rect>; label: string; kind: InlineCompletionHintStyle },
		styles: IObservable<{ background: string; border: string }>
	) {
		...
		const result = renderLines(
			...
			RenderOptions.fromEditor(this._editor).withSetWidth(false).withScrollBeyondLastColumn(0),
			[],
			line,
			true
		);
