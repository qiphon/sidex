...
export class GutterIndicatorMenuContent {
	...
	private _createHoverContent() {
		...
		const createOptionArgs = (options: {
			...
			commandId: string | IObservable<string>;
			commandArgs?: unknown[];
		}): FirstFnArg<typeof option> => {
			return {
				title: options.title,
				icon: options.icon,
				keybinding:
		...
		const alternativeCommand = this._data.alternativeAction
			? option(
					createOptionArgs({
		...
		const modelOptions = showModelEnabled
			? (this._data.modelInfo?.models.map((m: { id: string; name: string }) =>
					option({
						title: m.name,
		...
		const toggleCollapsedMode = this._inlineEditsShowCollapsed.map(showCollapsed =>
			showCollapsed
				? option(
						createOptionArgs({
							id: 'showExpanded',
	...
	private _getKeybinding(commandId: string | undefined) {
		...
		return observableFromEvent(this._contextKeyService.onDidChangeContext, () =>
		...
		n.div(
			...
			[
				...
				n.div({
					...
					ref: elem => {
						const keybindingLabel = _reader.store.add(
							new KeybindingLabel(elem, OS, {
								disableTitle: true,
...
function actionBar(actions: IAction[], options: IActionBarOptions) {
	return derived({ name: 'inlineEdits.actionBar' }, _reader =>
		n.div(
			{
