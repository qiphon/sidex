...
export const IExtensionResourceLoaderService = createDecorator<IExtensionResourceLoaderService>(
...
export interface IExtensionResourceLoaderService {
	...
	getExtensionGalleryResourceURL(
		galleryExtension: { publisher: string; name: string; version: string; targetPlatform?: TargetPlatform },
		path?: string
	...
	implements IExtensionResourceLoaderService
{
	...
	public async getExtensionGalleryResourceURL(
		{
			publisher,
			name,
			version,
	...
	async isExtensionGalleryResource(uri: URI): Promise<boolean> {
		await this._initPromise;
		return (
			!!this._extensionGalleryAuthority && this._extensionGalleryAuthority === this._getExtensionGalleryAuthority(uri)
		);
	}
