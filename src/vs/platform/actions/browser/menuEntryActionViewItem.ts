...
export function fillInActionBarActions(
	...
	const isPrimaryAction =
		typeof primaryGroup === 'string' ? (actionGroup: string) => actionGroup === primaryGroup : primaryGroup;
...
> extends ActionViewItem {
	...
	protected override getTooltip() {
		...
		if (!this._wantsAltCommand && this._menuItemAction.alt?.enabled) {
			...
			const altTitleSection = this._keybindingService.appendKeybinding(
				altTooltip,
		...
		if (this.label) {
			...
			if (this._options?.conversational) {
				this.label.textContent = localize(
					{ key: 'content2', comment: ['A label with keybindg like "ESC to dismiss"'] },
					'{1} to {0}',
					this._action.label,
					kb2
				);
...
export class DropdownWithDefaultActionViewItem extends BaseActionViewItem {
	...
	constructor(
		...
		const defaultActionId = options?.togglePrimaryAction
		...
		this._defaultAction = this._defaultActionDisposables.add(
			this._instaService.createInstance(MenuEntryActionViewItem, <MenuItemAction>defaultAction, {
		...
		this._dropdown = this._register(
			new DropdownMenuActionViewItem(submenuAction, submenuAction.actions, this._contextMenuService, dropdownOptions)
		);
	...
	private update(lastAction: MenuItemAction): void {
		...
		this._defaultAction = this._defaultActionDisposables.add(
			this._instaService.createInstance(MenuEntryActionViewItem, lastAction, {
	...
	override render(container: HTMLElement): void {
		...
		this._register(
			addDisposableListener(primaryContainer, EventType.KEY_DOWN, (e: KeyboardEvent) => {
		...
		this._register(
			addDisposableListener(dropdownContainer, EventType.KEY_DOWN, (e: KeyboardEvent) => {
				const event = new StandardKeyboardEvent(e);
	...
	constructor(
		...
		super(
			...
			action.actions.map(a =>
				a.id === Separator.ID ? SeparatorSelectOption : { text: a.label, isDisabled: !a.enabled }
			),
			0,
			contextViewService,
...
export function createActionViewItem(
	...
	action: IAction,
	options: IDropdownMenuActionViewItemOptions | IMenuEntryActionViewItemOptions | undefined
	...
	if (action instanceof MenuItemAction) {
		...
		if (action.item.isSelection) {
			...
			return instaService.createInstance(DropdownWithDefaultActionViewItem, action, {
				...options,
				togglePrimaryAction:
					typeof action.item.isSplitButton !== 'boolean' ? action.item.isSplitButton.togglePrimaryAction : false
			});
		} else {
