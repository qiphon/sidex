...
export const RawWorkbenchListScrollAtBoundaryContextKey = new RawContextKey<'none' | 'top' | 'bottom' | 'both'>(
	'listScrollAtBoundary',
	'none'
);
...
function toWorkbenchListOptions<T>(
	...
	const result: IListOptions<T> = {
		...
		multipleSelectionController:
			options.multipleSelectionController ?? disposables.add(new MultipleSelectionController(configurationService)),
...
abstract class ResourceNavigator<T> extends Disposable {
	...
	constructor(
		...
		this._register(
			Event.filter(this.widget.onDidChangeSelection, e => isKeyboardEvent(e.browserEvent))(e =>
				this.onSelectionFromKeyboard(e)
			)
		);
		this._register(
			this.widget.onPointer((e: { browserEvent: MouseEvent; element: T | undefined }) =>
				this.onPointer(e.element, e.browserEvent)
			)
		);
		this._register(
			this.widget.onMouseDblClick((e: { browserEvent: MouseEvent; element: T | undefined }) =>
	...
	private onSelectionFromKeyboard(event: ITreeEvent<any>): void {
		...
		const preserveFocus =
			typeof selectionKeyboardEvent.preserveFocus === 'boolean' ? selectionKeyboardEvent.preserveFocus : true;
...
export class WorkbenchObjectTree<T extends NonNullable<any>, TFilterData = void> extends ObjectTree<T, TFilterData> {
	...
	constructor(
		...
		const {
			...
			disposable
		} = instantiationService.invokeFunction(workbenchTreeDataPreamble, options as any);
...
export class WorkbenchAsyncDataTree<TInput, T, TFilterData = void> extends AsyncDataTree<TInput, T, TFilterData> {
	...
	override updateOptions(
		options: IWorkbenchAsyncDataTreeOptionsUpdate<IAsyncDataTreeNode<TInput, T> | null> = {}
	): void {
		super.updateOptions(options);
...
export class WorkbenchCompressibleAsyncDataTree<TInput, T, TFilterData = void> extends CompressibleAsyncDataTree<
	TInput,
	T,
	TFilterData
> {
...
function getDefaultTreeFindMode(configurationService: IConfigurationService) {
	...
	const deprecatedValue = configurationService.getValue<'simple' | 'highlight' | 'filter'>(
...
function workbenchTreeDataPreamble<
	T,
	TFilterData,
	TOptions extends IAbstractTreeOptions<T, TFilterData> | IAsyncDataTreeOptions<T, TFilterData>
>(
	accessor: ServicesAccessor,
	options: TOptions
	...
	const getTypeNavigationMode = () => {
		...
		const modeString = contextKeyService.getContextKeyValue<'automatic' | 'trigger'>(
