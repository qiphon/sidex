...
export class FileService extends Disposable implements IFileService {
	...
	registerProvider(scheme: string, provider: IFileSystemProvider): IDisposable {
		...
		providerDisposables.add(
			provider.onDidChangeCapabilities(() => this._onDidChangeFileSystemProviderCapabilities.fire({ provider, scheme }))
		);
	...
	private async doResolveFile(resource: URI, options?: IResolveFileOptions): Promise<IFileStat> {
		...
		return this.toFileStat(provider, resource, stat, undefined, !!resolveMetadata, (stat, siblings) => {
			...
			if (
				...
				trie.findSuperstr(
					stat.resource.with(
						{ query: null, fragment: null } /* required for https://github.com/microsoft/vscode/issues/128151 */
					)
				)
			) {
				return true;
	...
	async createFile(
		resource: URI,
		bufferOrReadableOrStream: VSBuffer | VSBufferReadable | VSBufferReadableStream = VSBuffer.fromString(''),
	...
	async writeFile(
		...
		try {
			...
			let { stat, buffer: bufferOrReadableOrStreamOrBufferedStream } = await this.validateWriteFile(
				provider,
				resource,
	...
	private async peekBufferForWriting(
		provider: IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithOpenReadWriteCloseCapability,
		bufferOrReadableOrStream: VSBuffer | VSBufferReadable | VSBufferReadableStream
	): Promise<VSBuffer | VSBufferReadable | VSBufferReadableStream | VSBufferReadableBufferedStream> {
	...
	private async validateWriteFile(
		...
		stat: IStat | undefined;
		buffer: VSBuffer | VSBufferReadable | VSBufferReadableStream | VSBufferReadableBufferedStream | undefined;
	}> {
	...
	private async doWriteBuffered(
		...
		readableOrStreamOrBufferedStream: VSBufferReadable | VSBufferReadableStream | VSBufferReadableBufferedStream
	): Promise<void> {
		return this.writeQueue.queueFor(
			resource,
			async () => {
	...
	private async doWriteStreamBufferedQueued(
		provider: IFileSystemProviderWithOpenReadWriteCloseCapability,
		handle: number,
		streamOrBufferedStream: VSBufferReadableStream | VSBufferReadableBufferedStream
	): Promise<void> {
		let posInFile = 0;
