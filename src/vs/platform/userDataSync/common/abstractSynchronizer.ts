...
export abstract class AbstractSynchroniser extends Disposable implements IUserDataSynchroniser {
	...
	async sync(
		refOrUserData: string | IUserData | null,
		preview: boolean = false,
		userDataSyncConfiguration: IUserDataSyncConfiguration = this.getUserDataSyncConfiguration(),
		headers: IHeaders = {}
		...
		try {
			...
			for (const resourcePreviewResult of resourcePreviewResults) {
				...
				resourcePreviews.push([
					resourcePreviewResult,
					{ ...acceptResult, remoteChange: remoteChange !== Change.None ? remoteChange : Change.Modified }
				]);
			}
	...
	protected async getLatestRemoteUserData(
		refOrLatestData: string | IUserData | null,
	...
	private async updateSyncResourcePreview(
		resource: URI,
		updateResourcePreview: (resourcePreview: IEditableResourcePreview) => Promise<IEditableResourcePreview>
	): Promise<void> {
		if (!this.syncPreviewPromise) {
			return;
		}
	...
	private async doApply(force: boolean): Promise<SyncStatus> {
		...
		await this.applyResult(
			preview.remoteUserData,
			preview.lastSyncUserData,
			preview.resourcePreviews.map(resourcePreview => [resourcePreview, resourcePreview.acceptResult!]),
			force
		);
	...
	protected parseSyncData(content: string): ISyncData {
		...
		throw new UserDataSyncError(
			localize('incompatible sync data', 'Cannot parse sync data as it is not compatible with the current version.'),
	...
	private async getUserData(lastSyncData: IRemoteUserData | null): Promise<IUserData> {
		const lastSyncUserData: IUserData | null = lastSyncData
			? { ref: lastSyncData.ref, content: lastSyncData.syncData ? JSON.stringify(lastSyncData.syncData) : null }
			: null;
	...
	protected async backupLocal(content: string): Promise<void> {
		...
		return this.userDataSyncLocalStoreService.writeResource(
			...
			JSON.stringify(syncData),
			new Date(),
			this.syncResource.profile.isDefault ? undefined : this.syncResource.profile.id
		);
	}
...
export abstract class AbstractFileSynchroniser extends AbstractSynchroniser {
	...
	protected async updateLocalFileContent(
		...
		try {
			...
			if (
				(e instanceof FileOperationError && e.fileOperationResult === FileOperationResult.FILE_NOT_FOUND) ||
