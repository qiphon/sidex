...
export function updateIgnoredSettings(
	...
	if (ignoredSettings.length) {
		...
		settingsToAdd.forEach(
			s => (targetContent = addSetting(s.setting!.key, sourceContent, targetContent, formattingOptions))
		);
	}
	return targetContent;
}
...
function insertAtLocation(
	...
	if (location.index === -1) {
		...
		edits = getEditToInsertAtLocation(content, key, value, location, tree, formattingOptions).map(
			edit => withFormatting(content, edit, formattingOptions)[0]
		);
	}
	...
	if (location.insertAfter) {
		...
		if (node.setting) {
			...
			const isPreviouisSettingIncludesComment =
				previousSettingCommaOffset !== undefined && previousSettingCommaOffset > node.endOffset;
			edits.push({
		...
		const content =
			(tree[location.index - 1] && !tree[location.index - 1].setting /* previous node is comment */ ? eol : '') +
			newProperty +
