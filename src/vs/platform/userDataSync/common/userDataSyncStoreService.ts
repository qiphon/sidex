	...
	implements IUserDataSyncStoreManagementService
{
	...
	constructor(
		...
		this._register(
			Event.filter(
				storageService.onDidChangeValue(StorageScope.APPLICATION, SYNC_SERVICE_URL_TYPE, disposable),
	...
	protected toUserDataSyncStore(
		configurationSyncStore: (ConfigurationSyncStore & { web?: ConfigurationSyncStore }) | undefined
		...
		if (
			...
			Object.keys(configurationSyncStore.authenticationProviders).every(authenticationProviderId =>
				Array.isArray(configurationSyncStore.authenticationProviders[authenticationProviderId].scopes)
			)
		) {
			...
			return {
				...
				canSwitch,
				authenticationProviders: Object.keys(syncStore.authenticationProviders).reduce<IAuthenticationProvider[]>(
					(result, id) => {
...
export class UserDataSyncStoreClient extends Disposable {
	...
	async getAllCollections(headers: IHeaders = {}): Promise<string[]> {
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.getAllCollections' },
			[],
	...
	async createCollection(headers: IHeaders = {}): Promise<string> {
		...
		const context = await this.request(
			url,
			{ type: 'POST', headers, callSite: 'userDataSync.createCollection' },
			[],
			CancellationToken.None
		);
	...
	async deleteCollection(collection?: string, headers: IHeaders = {}): Promise<void> {
		...
		await this.request(
			url,
			{ type: 'DELETE', headers, callSite: 'userDataSync.deleteCollection' },
			[],
			CancellationToken.None
		);
	}
	...
	async getAllResourceRefs(resource: ServerResource, collection?: string): Promise<IResourceRefHandle[]> {
		...
		const context = await this.request(
			uri.toString(),
			{ type: 'GET', headers, callSite: 'userDataSync.getAllResourceRefs' },
			[],
			CancellationToken.None
		);
	...
	async resolveResourceContent(
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.resolveResourceContent' },
			[],
			CancellationToken.None
		);
	...
	async deleteResource(resource: ServerResource, ref: string | null, collection?: string): Promise<void> {
		...
		await this.request(
			url,
			{ type: 'DELETE', headers, callSite: 'userDataSync.deleteResource' },
			[],
			CancellationToken.None
		);
	}
	...
	async deleteResources(): Promise<void> {
		...
		await this.request(
			url,
			{ type: 'DELETE', headers, callSite: 'userDataSync.deleteResources' },
			[],
			CancellationToken.None
		);
	}
	...
	async readResource(
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.readResource' },
			[304],
			CancellationToken.None
		);
	...
	async writeResource(
		...
		const context = await this.request(
			url,
			{ type: 'POST', data, headers, callSite: 'userDataSync.writeResource' },
			[],
			CancellationToken.None
		);
	...
	async manifest(oldValue: IUserDataManifest | null, headers: IHeaders = {}): Promise<IUserDataManifest | null> {
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.manifest' },
			[304],
			CancellationToken.None
		);
	...
	async getLatestData(headers: IHeaders = {}): Promise<IUserDataSyncLatestData | null> {
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.getLatestData' },
			[],
			CancellationToken.None
		);
	...
	async getActivityData(): Promise<VSBufferReadableStream> {
		...
		const context = await this.request(
			url,
			{ type: 'GET', headers, callSite: 'userDataSync.getActivityData' },
			[],
			CancellationToken.None
		);
	...
	private async request(
		...
		const isSuccess =
			isSuccessContext(context) || (context.res.statusCode && successCodes.includes(context.res.statusCode));
		let failureMessage = '';
		if (isSuccess) {
